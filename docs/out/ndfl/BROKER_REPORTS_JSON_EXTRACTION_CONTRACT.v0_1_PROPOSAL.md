...
Event-level ledgers should move to `BROKER_REPORTS_SOURCE_FACTS_SCHEMA.v0_PROPOSAL` instead of being embedded deeply into `broker_reports_extraction_v0_1`. This proposal should keep only additive manifest, alignment, evidence-wrapper and review-state hooks needed to link extraction output to that schema.
...
## 6. Source Facts Linkage

Current fields are total-oriented. Do not expand `extracted_tax_facts` into a full event ledger.

Add a lightweight linkage to the separate source facts schema:

```json
{
  "source_facts_schema_ref": {
    "schema_version": "broker_reports_source_facts_v0_proposal",
    "source_facts_set_id": null,
    "source_fact_refs": [],
    "generation_status": "not_started | source_facts_only | failed"
  }
}
```

The separate schema owns:

- income events;
- securities operation events;
- withholding events;
- fee events;
- currency events;
...
- review-only and calculation-required markers.

Why:

Declaration-oriented review needs event-level facts before totals, but the extraction contract should not become the declaration model or the full event ledger.
...
- `broker_reports_extraction_v0_1` as a small additive proof schema with references; and
- a separate source-facts schema feeding the declaration model.

Post-human-review preference: use the separate source-facts schema.
