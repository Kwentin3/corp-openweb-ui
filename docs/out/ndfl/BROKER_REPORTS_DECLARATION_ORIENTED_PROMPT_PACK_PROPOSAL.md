...
- period-aware official source set required before declaration mapping readiness;
- document taxonomy required before using any document as source evidence or methodology;
...
- `BROKER_REPORTS_DOCUMENT_TAXONOMY.v0`;
...
- if a document is an instruction, template, example, help article or public layout sample, do not use it as taxpayer source evidence.
...
- `BROKER_REPORTS_SOURCE_FACTS_SCHEMA.v0_PROPOSAL` output;
- source fact event ledgers;
- raw labels/values;
- source refs;
- confidence;
- methodology status;
- declaration relevance;
- calculation-required markers.
...
- do not output declaration model fields directly from source fact extraction.
...
- period-aware official requirements registry;
- official source set ID;
...
- `BROKER_REPORTS_NDFL_DECLARATION_DATA_MODEL.v0_1_PROPOSAL`.

Failure behavior:

- if official source set is missing, mark `official_source_gap`.
- if customer methodology is missing, mark `requires_customer_methodology` and ask questions.
- keep official-source gaps and methodology gaps separate.
...
- if an official source set is absent for the pilot tax year, mark `official_source_gap`.
...
## 13. Post-Human-Review Refinement Status

```text
PROMPT_PACK_REFINED_AGAINST_SOURCE_FACTS_SCHEMA
DOCUMENT_TAXONOMY_REQUIRED
OFFICIAL_REQUIREMENT_REFS_REQUIRED
