...
```text
Stage 2 Tranche 1: OpenWebUI media transcription module
```
...
В рамках этого аудита из PRD-1 выделен первый оплачиваемый срез: модуль транскрибации аудио/видео внутри OpenWebUI. Этот срез опирается на уже реализованный и проверенный путь:
...
Остальные направления PRD-1 остаются будущими работами и не включаются автоматически в текущую сумму 150 000 рублей.
...
Стоимость работ по данному ограниченному этапу: 150 000 рублей.

Эта сумма относится только к перечисленному ниже объему работ. Она не покрывает весь PRD-1 / Practical Stage 2. Внешние расходы на сервер, хранение, платные API, STT-провайдера, web-search или другие подписки могут оплачиваться отдельно, если это требуется эксплуатационной моделью.
...
7. Настройка runtime wiring между OpenWebUI, static assets и приватным
   STT-sidecar.
8. Проведение runtime, Playwright и unit/smoke проверок по MVP-сценариям.
9. Подготовка и актуализация технической документации, отчетов и acceptance
...
web-search, OCR, broker-report workflow, hard billing, full AD/SSO и data
masking оформляются отдельными будущими траншами.

Стоимость:
Стоимость работ по данному ограниченному этапу составляет 150 000 рублей.
Внешние расходы на сервер, хранение, платные API, STT-провайдера и другие
эксплуатационные сервисы не входят в указанную стоимость, если иное не
...
транскрибации аудио/видео для корпоративного OpenWebUI в рамках первого
ограниченного среза Stage 2.
...
медиафайлов через self-hosted ffmpeg.wasm assets, runtime wiring и проверка
MVP-сценариев.
...
STT-провайдера остаются на серверной стороне.

Заказчик принял результат работ в рамках ограниченного объема Stage 2 Tranche 1.
Стоимость выполненных работ составляет 150 000 рублей.
...
экспорту транскриптов, web-search, OCR, broker-report workflow, hard billing,
AD/SSO, data masking и другим направлениям PRD-1 оформляются отдельными
траншами при необходимости.
```
...
Сумма 150 000 рублей обоснована как коммерческая стоимость уже выполненного и доказанного первого функционального среза Stage 2: архитектура, backend-sidecar, provider adapter, OpenWebUI Action Function, static loader UI action, browser ffmpeg.wasm normalization, runtime proof, Playwright proof, tests/smoke и документационная фиксация.
...
- другой функциональный блок PRD-1 по согласованию: web-search, OCR/layout-aware PDF pilot, broker reports / 3-НДФЛ, workspaces/groups/prompts или analytics/cost visibility.
