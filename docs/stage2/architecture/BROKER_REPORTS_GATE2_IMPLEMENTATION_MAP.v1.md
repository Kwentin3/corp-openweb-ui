...
| PDF Document AI boundary | `pdf_document_ai.PdfDocumentExtractor` selected by `PdfDocumentExtractorFactory.create` | exact owner-checked OpenWebUI file bytes after preflight | typed `PDF_DOCUMENT_AI_NOT_CONFIGURED`, or configured provider-neutral extraction persisted in ArtifactStore and projected as owner-scoped Full Source | provider/model response knowledge outside adapter, local extraction, custom intake/action, retry, fallback, caller byte injection |
