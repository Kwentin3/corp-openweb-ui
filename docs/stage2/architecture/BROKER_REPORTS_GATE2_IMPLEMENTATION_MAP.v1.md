...
| PDF layout evidence | `pdf_layout_units.PdfLayoutUnitBuilder` through `FullSourceArtifactBuilder` | parsed page layout | provenance-rich private units/proposals | direct public-reader use, provider proposal as truth |
...
| PDF assembly | `CanonicalNormalizer._adapt_pdf` plus `pdf_source_atom_accounting` | PDF units and terminal table proposals | common containers/nodes/tables/provenance | unaccounted parser atoms, VLM authority |
...
| completeness | `canonical_artifact.assess_canonical_completeness` and PDF atom accounting | common logical artifact | counts-only pass/fail reasons | activation of non-empty zero-node artifacts |
