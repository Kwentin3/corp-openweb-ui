...
| Managed semantic alias/readability policy | Gate 2 model-visible presentation inside the existing packet/Choice owners | Emit a local key only for a visible cross-reference or response; pair keys with evidence-owned labels and Pack titles, factor common eligibility relationships and use ambiguous `unclassified` only when two or more distinct type meanings remain plausible; one plausible type without one safe choice is a compatibility stop | Managed Semantic Decision Context GOAL 4 implements the audited projection as a non-active packet sidecar; complete exact bindings and refs remain private, and four zero-option expected-answer concerns remain an explicit compatibility stop | Existing Context V2 packet sidecar; later Choice V2 and Context Linter | Frozen local proof retains all exact bindings and factors duplicate visible occurrences; provider calls are zero and the active request route is unchanged |
| Managed Semantic Context V2 contract | Gate 2 model-visible successor boundary inside the existing packet/Choice owners | Closed readable source/type/relationship/choice/reason view; available types come from the exact Registry snapshot with Pack-baseline parity, while Compiler outputs are parity-only; response keys map to exact canonical Choice through a packet-owned private mapping receipt that is never model input, and a future V2 Context Linter must separately seal the complete request | Packet-owned candidate plus private mapping receipt implemented non-active; packet runtime changes additively only by returning these sidecars, while the active V6 payload, historical Slim v2, Prompt, active Choice and provider route remain unchanged; count-one, frozen reason-compatibility and provider-profile compatibility stops remain | Existing PacketFactory and Pack projection owner; future Choice V2, Context Linter, evidence replay and request route | Managed Semantic Decision Context GOAL 4 local implementation accepted only after exact parity/tamper tests; provider projection, replay proof and benchmark conformance not claimed |
