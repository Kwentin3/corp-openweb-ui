...
| Inactive `broker_reports_managed_document_v2` logical-row contour | `ManagedPdfDocumentV2Factory` coordinating sole `FullSourceArtifactFactory`, `LogicalRowTableFactory` recovery and exact-schema-pinned `ManagedDocumentContractV2Validator` acceptance owners | Raw PDF bytes plus private artifact identity -> internal sole complete FullSource PDF projection -> ordered document blocks -> row-first TABLE with roles, hierarchy, ordered entries, optional logical columns, private geometry and exact-once table-word ownership | Versioned/private Global Gate 1 candidate; the ref-derived internal document ID is private diagnostic state and never enters the model view; no canonical grid/cell/span core, financial meaning, provider input or product route | DOC6 offline recovery, independent visual-gold comparison and LLM Document View v2 only | Additive inactive DOC6 implementation; Managed Document v1, FullSource owner, active routes and generated bundles unchanged |
| Inactive `broker_reports_llm_document_view_v2` | `ManagedDocumentLlmViewV2Factory`; independent acceptance by `ManagedDocumentLlmViewV2Auditor` | Deterministic row-oriented model-readable projection of an already validated Managed Document v2 | Derived/non-authoritative; content and safe pointers only, private geometry/ownership omitted | DOC6 offline row-parity evidence only | Additive inactive candidate; no request, provider, qualification or product consumer |
...
| Semantic visual-table contract | Gate 1 | One declared crop -> `description` plus source-visible logical `rows` | `broker_reports_semantic_table_transcription_v1`; Gemini master; OpenAI optional control/fallback; private | Deterministic semantic validator/materializer | Closed for the Goal 5-qualified numeric profile; activated only by the atomic release valve set |
| Deterministic semantic materialization | Gate 1 | Semantic rows -> application-owned envelope and rectangular logical table | Code-owned indexes, null padding, span 1 and explicit semantic origin; no physical topology claim | Gate 2 table-package factory for the qualified profile | Implemented, validated and included in the atomic semantic release contour |
...
| Managed Financial Domain semantic asset family | Gate 2 semantic assets | Versioned Financial Semantic Pack, managed Skill/Prompt/Tool identities and one versioned decision-reason catalog dependency | Family v2 `1.1.0` remains an inactive repository draft with exact v1 rollback baseline; its closed candidate snapshot is packaged only through the existing single model-assets loader; live publish/readback/rollback remain explicit gaps | Existing V6 packet/context projection and future extension of the atomic release contour | Same family selected by GOAL 0 and extended additively by GOAL 1; default active profile unchanged, no second registry, loader or GUI |
| Managed semantic alias/readability policy | Gate 2 model-visible presentation inside the existing packet/Choice owners | Emit a local key only for a visible cross-reference or response; pair keys with evidence-owned labels and Pack titles, factor common eligibility relationships and select each unclassified reason by plausible distinct type count plus uniquely safe complete-choice count, never raw Compiler counts | Managed Semantic Decision Context GOAL 4 preserves the non-active V2.0 completeness baseline; GOAL 6 adds the inactive count-one reason candidate; GOAL 8 implements the current non-active V2.1 minimal candidate without changing the active route | Existing V2.0 packet sidecar remains version-pinned historical evidence; current V2.1 follows the Minimal Model Surface and Outcome Taxonomy contracts | Frozen V2.1 proof retains 45 literal occurrences and 59 bindings, emits no consumerless aliases, records provider calls zero, and leaves active Prompt, Pack, Choice, Packet and benchmark manifests unchanged |
| Managed Semantic Context V2.0 completeness baseline | Gate 2 model-visible historical candidate inside the existing packet/Choice owners | Closed readable source/type/relationship/choice/reason view; available types come from the exact Registry snapshot with Pack-baseline parity, while Compiler outputs are parity-only; response keys map to exact canonical Choice through a packet-owned private mapping receipt that is never model input | Packet-owned candidate plus private mapping receipt implemented non-active; it is exact version-pinned completeness evidence, not the future minimal model-surface target; active V6 payload, Prompt, active Choice and provider route remain unchanged | Existing PacketFactory and Pack projection owner only | Managed Semantic Decision Context GOAL 4 local implementation accepted only after exact parity/tamper tests; provider projection, replay proof and benchmark conformance not claimed |
| [Context V2.1](../contracts/BROKER_REPORTS_GATE2_LLM_SEMANTIC_CONTEXT.v2.1.md) | Gate 2 current non-active minimal candidate inside the existing Packet owner | Five exact model-visible blocks plus private restoration of source occurrences, type keys, choices and every binding | Existing PacketFactory builds one candidate/receipt; Context Linter seals exact Prompt + candidate + Choice schema; GOAL 11 composes local adapters/materialization/persistence/report owners; GOAL 12 adds qualification-only one-attempt transport/evidence under the same owners | Inactive Choice, linter, local proof and explicit qualification consumers only; no product runtime consumer | `active=false`, `transport_eligible=false`; local 12-path replay exact; terminal smoke admitted no benchmark-eligible model and cannot admit production |
| [Context V2.1 Local Choice](../contracts/BROKER_REPORTS_GATE2_FINANCIAL_SEMANTIC_LOCAL_CHOICE.v2.1.md) | Gate 2 inactive response profile inside the existing Choice owner | Closed `choice_N`/unclassified response and exact receipt-only restoration to the unchanged V6 minimal Choice shape | Existing Choice factory pins Context view, mapping receipt and active Choice hash; active/historical Choice schemas stay exact | Local parser, provider-neutral linter, candidate-only Expansion and GOAL 12 qualification; no active runtime consumer | GOAL 11 proves all three reasons locally; terminal GOAL 12 smoke changed no Choice authority and admitted no model |
| [Context V2.1 Budget Model Smoke v1](../contracts/BROKER_REPORTS_GATE2_CONTEXT_V2_1_BUDGET_MODEL_SMOKE.v1.md) | Gate 2 Qualification | Freeze and execute at most one attempt for each of 12 provider/case slots, preserve exact private evidence and issue separate technical/semantic verdicts | Existing fixture/linter/request/budget/adapter/client/Choice/expansion/materialization/evidence/report owners; thin plan/coordinator; exact direct-provider transport resolved from the enabled OpenWebUI connection; HMAC-sealed external ledger, per-slot exclusive claims and repository-scoped execution-lock tag | GOAL 13 may consume only eligible model identities; product runtime consumes nothing | The pre-call Actions gate passed; terminal execution used `8` submissions with zero retry/repair/fallback, OpenAI/Anthropic semantic failures and four Google pretransport failures; no model is eligible |
| [Minimal Model Surface v1](../contracts/BROKER_REPORTS_GATE2_MINIMAL_MODEL_SURFACE.v1.md) | Gate 2 model-visible field-eligibility and exact managed-string selection boundary inside Semantic Matcher | Decide whether each model-visible field is necessary to the semantic choice; keep full source/Pack/Reason/Choice authority backend-owned and prohibit fields without a decision consumer | GOAL 5 selected existing mappings; GOAL 7 implements the inactive Pack/reason profile; GOAL 8 the candidate/private receipt; GOAL 9 the response profile; GOAL 10 the sealed request; GOAL 11 the local proof; GOAL 12 the terminal qualification smoke | Existing Prompt, Evidence Bundle, Pack, Reason Catalog, Packet, Choice, Context Linter, request, budget, adapter, client, materialization, evidence and report owners | Local proof is exact; terminal qualification admitted no model; activation remains absent |
| [Outcome Taxonomy v1](../contracts/BROKER_REPORTS_GATE2_OUTCOME_TAXONOMY.v1.md) | Gate 2 future semantic outcome vocabulary and corrected benchmark-answer authority | Define mutually exclusive typed `1/1`, no-type `0/0`, ambiguous-type `2+/0`, and single-type/no-safe-record `1/0` states; keep insufficient source context and technical failure outside semantic reasons | GOAL 6 added one inactive catalog v2 reason candidate; GOAL 7 packages/projects all three exact reason cards; GOAL 9 admits them only in inactive V2.1 Choice; GOAL 11 materializes them on the candidate path; GOAL 12 executed the same four audited smoke states | Existing reason catalog ID, family v3, Choice, decision and Expansion authorities only | Active V6 still has two reasons; GOAL 12 was qualification-only and did not activate the third reason |
| V6 financial semantic Context Linter (historical staged program) | Gate 2 | Validate and seal the complete version-pinned Prompt + candidate context + Local Choice before request construction | Qualification-only candidate boundary; packet/Choice remain separate authorities and current runtime remains unchanged | Existing request builder candidate profile | Historical staged GOAL 3 implemented and locally total; non-active, provider calls zero |
| V6 Slim model diagnostic | Gate 2 qualification evidence | Orchestrate the exact Nano/Haiku canonical and Nano reversed six-cell smoke over two frozen cases | Evidence-only; reuses packet/Choice/linter/client/materialization owners and cannot admit or activate | Human acceptance decision only | Historical Slim-program GOAL 4 terminal failed: technical pipeline passed, Haiku unclassified reason missed, benchmark not run |
...
| Semantic visual-table contract | Closed for the Goal 5-qualified numeric profile | `broker_reports_semantic_table_transcription_v1` is the authoritative model-facing shape. The atomic release valve selects Gemini master plus deterministic materialization/downstream migration; OpenAI stays disabled by default, geometry and system metadata are application-owned, and Markdown/local OCR are absent from the runtime boundary. |
