...
| Managed Financial Domain semantic asset family | Gate 2 semantic assets | Versioned Financial Semantic Pack, managed Skill/Prompt/Tool identities and one versioned decision-reason catalog dependency | Family v2 `1.1.0` is a repository-managed inactive draft with exact v1 rollback baseline; Workspace surface remains authoring/inspection only, and isolated live draft plus full-family publish/readback/rollback remain explicit gaps | Existing V6 packet/context projection and future extension of the atomic release contour | Same family selected by GOAL 0 and extended additively by GOAL 1; v1/runtime byte-exact, no second registry or GUI |
| Managed semantic alias/readability policy | Gate 2 model-visible presentation inside the existing packet/Choice owners | Emit a local key only for a visible cross-reference or response; pair keys with evidence-owned labels and Pack titles, factor common eligibility relationships and use ambiguous `unclassified` only when two or more distinct type meanings remain plausible; one plausible type without one safe choice is a compatibility stop | GOAL 2 documentation-only audit; complete exact bindings and refs remain backend-owned, Context V2 is not implemented and four zero-option cases retain an explicit compatibility stop | Future versioned Context V2 contract, versioned extension of the existing type-card projection and existing Context Linter | Frozen audit passed with provider/runtime changes zero; 22 unused value aliases, 14 unused structural aliases and 24 duplicate binding occurrences identified |
| Managed Semantic Context V2 contract | Gate 2 model-visible successor boundary inside the existing packet/Choice owners | Closed readable source/type/relationship/choice/reason view; available types come from the exact Registry snapshot with Pack-baseline parity, while Compiler outputs are parity-only; response keys restore exact canonical Choice through a packet-owned mapping receipt and the Context Linter separately seals the complete request | Versioned non-active contract only; current V6 packet, historical Slim v2, Prompt, Pack/reason bytes, provider and runtime unchanged; count-one, frozen reason-compatibility and provider-profile compatibility stops remain | Future non-active implementation in the existing packet, Pack projection, Choice and Context Linter authorities | GOAL 3 contract accepted locally; implementation, provider projection, replay proof and benchmark conformance not claimed |
