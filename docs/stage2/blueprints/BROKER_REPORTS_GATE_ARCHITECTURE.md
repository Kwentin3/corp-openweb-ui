...
| [Context V2.1](../contracts/BROKER_REPORTS_GATE2_LLM_SEMANTIC_CONTEXT.v2.1.md) | Gate 2 current non-active minimal candidate inside the existing Packet owner | Five exact model-visible blocks plus private restoration of source occurrences, type keys, choices and every binding | Existing PacketFactory builds one candidate/receipt from the GOAL 7 minimal projection; the existing Context Linter authority seals the exact Prompt + candidate + Choice schema through additive `create_context_v2_1`; active V6 payload and historical linter path stay unchanged | Inactive Choice and linter consumers only; no adapter, runtime, persistence or replay consumer | `active=false`, `transport_eligible=false`, sealed maximum 3,522 bytes, aggregate 34,389 bytes, provider calls zero |
| [Context V2.1 Local Choice](../contracts/BROKER_REPORTS_GATE2_FINANCIAL_SEMANTIC_LOCAL_CHOICE.v2.1.md) | Gate 2 inactive response profile inside the existing Choice owner | Closed `choice_N`/unclassified response and exact receipt-only restoration to the unchanged V6 minimal Choice shape | Existing Choice factory pins Context view, mapping receipt and active Choice hash; active/historical Choice schemas stay exact | Local parser plus provider-neutral linter consumption only; no Expansion, adapter or runtime consumer | GOAL 9 implemented inactive; GOAL 10 consumes exact schema without inventing it; provider calls zero; **STOP before GOAL 11** |
| [Minimal Model Surface v1](../contracts/BROKER_REPORTS_GATE2_MINIMAL_MODEL_SURFACE.v1.md) | Gate 2 model-visible field-eligibility and exact managed-string selection boundary inside Semantic Matcher | Decide whether each model-visible field is necessary to the semantic choice; keep full source/Pack/Reason/Choice authority backend-owned and prohibit fields without a decision consumer | GOAL 5 selected existing mappings; GOAL 7 implements the inactive Pack/reason profile; GOAL 8 the non-active candidate/private receipt; GOAL 9 the inactive response profile; GOAL 10 the inactive provider-neutral sealed request and lint receipt | Existing Prompt, Evidence Bundle, Pack, Reason Catalog, Packet, Choice and Context Linter owners; [family v3](../contracts/BROKER_REPORTS_OPENWEBUI_FINANCIAL_DOMAIN_ASSET_FAMILY.v3.md) packages catalog v2 | Provider projection, adapter consumption, persistence, replay and activation remain absent; GOAL 11 waits for reviewed, green, merged GOAL 10 |
| [Outcome Taxonomy v1](../contracts/BROKER_REPORTS_GATE2_OUTCOME_TAXONOMY.v1.md) | Gate 2 future semantic outcome vocabulary and corrected benchmark-answer authority | Define mutually exclusive typed `1/1`, no-type `0/0`, ambiguous-type `2+/0`, and single-type/no-safe-record `1/0` states; keep insufficient source context and technical failure outside semantic reasons | GOAL 6 added one inactive catalog v2 reason candidate; GOAL 7 packages/projects all three exact reason cards; GOAL 9 admits them only in inactive V2.1 Choice; GOAL 10 seals the provider-neutral request | Existing reason catalog ID, family v3, Choice and Context Linter authorities only | Active V6 still has two reasons; provider transport waits for GOAL 11; provider calls zero |
