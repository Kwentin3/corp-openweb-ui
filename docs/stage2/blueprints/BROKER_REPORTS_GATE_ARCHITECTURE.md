...
| Managed semantic alias/readability policy | Gate 2 model-visible presentation inside the existing packet/Choice owners | Emit a local key only for a visible cross-reference or response; pair keys with evidence-owned labels and Pack titles, factor common eligibility relationships and select each unclassified reason by plausible distinct type count plus uniquely safe complete-choice count, never raw Compiler counts | Managed Semantic Decision Context GOAL 4 preserves the non-active V2.0 completeness baseline; GOAL 6 adds the inactive count-one reason candidate and proves the four zero-choice semantic counts `2,1,1,1` without changing the active route | Existing V2.0 packet sidecar remains version-pinned historical evidence; future V2.1 work follows the Minimal Model Surface and Outcome Taxonomy contracts | Three historical ambiguous reasons are corrected only in an additive frozen audit identity; provider calls are zero and active Prompt, Pack, Choice, Packet and benchmark manifests are unchanged |
...
| [Minimal Model Surface v1](../contracts/BROKER_REPORTS_GATE2_MINIMAL_MODEL_SURFACE.v1.md) | Gate 2 model-visible field-eligibility and exact managed-string selection boundary inside Semantic Matcher | Decide whether each future model-visible field is necessary to the semantic choice; keep full source/Pack/Reason/Choice authority backend-owned and prohibit fields without a decision consumer | Documentation-only GOAL 5 contract; selects existing Pack `examples[0]`, `counterexamples[0]`, the unique direct rule against the only other current visible type, and the exact first sentence of catalog `meaning`; creates no renderer, projection, response profile, linter, request route or runtime authority | Existing Prompt, Evidence Bundle, Pack, Reason Catalog, Packet and Choice owners; GOAL 6 audits taxonomy/count-one, GOAL 7 implements the selected managed mappings, and GOAL 8 builds only the PacketFactory V2.1 candidate plus private receipt | Contracted future target only; **STOP before GOAL 9:** a separately authorized/versioned V2.1 response profile must exist in the Choice authority, or the program must be amended; GOAL 9 is linter-only and consumes rather than builds the P01-P18 Choice schema |
| [Outcome Taxonomy v1](../contracts/BROKER_REPORTS_GATE2_OUTCOME_TAXONOMY.v1.md) | Gate 2 future semantic outcome vocabulary and corrected benchmark-answer authority | Define mutually exclusive typed `1/1`, no-type `0/0`, ambiguous-type `2+/0`, and single-type/no-safe-record `1/0` states; keep insufficient source context and technical failure outside semantic reasons | GOAL 6 adds one inactive catalog v2 reason candidate and one frozen corrected expectation audit; historical manifests and active V6 Choice remain immutable | Existing reason catalog ID, qualification benchmark owner and future V2.1 Choice authority only | GOAL 7 may project the inactive catalog v2 fields; no transport until separate V2.1 Choice-profile authorization, provider calls zero, full benchmark not run |
