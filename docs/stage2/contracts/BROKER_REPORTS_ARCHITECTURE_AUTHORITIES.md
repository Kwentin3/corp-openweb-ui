...
| Provider transport-request construction | [`Gate2OpenWebUIRequestBuilder.build`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/gate2_model_requests.py) | provider/model request profiles consuming an already linted logical request | structured model client; delegating evidence helper | historical profiles validate then delegate; inactive V2.1 is not a transport profile before GOAL 11 | direct `form_data` assembly in evidence or qualification, or provider fields added by the Context Linter |
...
| Semantic choice | [`Gate2FinancialSemanticV6ChoiceContractFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/gate2_financial_semantic_v6_choice.py) | active [V6 Choice](./BROKER_REPORTS_GATE2_FINANCIAL_SEMANTIC_CHOICE_V6.md), historical [Local Choice v1](./BROKER_REPORTS_GATE2_FINANCIAL_SEMANTIC_LOCAL_CHOICE.v1.md), inactive [Local Choice V2.1](./BROKER_REPORTS_GATE2_FINANCIAL_SEMANTIC_LOCAL_CHOICE.v2.1.md) | active request builder/expansion/evidence; inactive V2.1 parser and exact provider-neutral linter sealing only | active exact-ID bytes and historical Local v1 remain pinned; V2.1 restores only through its private receipt and public validation pins exact model-order schema bytes | alternative Choice factory/schema authority, index-derived restoration or model-generated records/bindings |
...
7. **STOP before GOAL 11:** provider projection and local adapter proof require
   reviewed, green, merged GOAL 10.
...
The profile now has only one inactive request consumer:
`Gate2FinancialSemanticV6ContextLinterFactory.create_context_v2_1`. The third
reason remains outside active V6 Expansion/materialization. Provider calls,
adapter changes, managed-asset changes, benchmark runs and runtime activation
are zero. Provider projection and local adapter proof remain stopped before
GOAL 11.
...
Packet construction, Choice-owned GOAL 9 response profile and the GOAL 10
provider-neutral Context Linter are its only consumers. None is provider
transport or runtime. The next boundary is the GOAL 11 provider-projection
proof STOP.
