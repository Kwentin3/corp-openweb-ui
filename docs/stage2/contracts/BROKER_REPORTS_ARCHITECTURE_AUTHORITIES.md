...
| Inactive PDF -> Managed Document v2 orchestration | [`ManagedPdfDocumentV2Factory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/managed_pdf_document_v2.py) | [Managed Document v2](./BROKER_REPORTS_MANAGED_DOCUMENT.v2.md); legacy `create(schema).build(...)` remains unchanged, while `create_adjudicated_for_openwebui(schema, request)` resolves the enabled Admin provider through the existing OpenWebUI connection owner before returning a builder whose build accepts only original PDF bytes, private source identity, task ID and DPI | offline DOC6 recovery proof, adjudicated row-oriented view and private inactive whole-table projection output only | exact FullSource runs once; the exact factory-created document adjudicator alone owns admission; its same-call recovery reaches private Managed assembly only on `COVERAGE_COMPLETE`; that same owner binds each table part to exact whole FullSource units and seals real unit/checksum/atom/word coverage; the private whole-table projection may be returned from the same call after seal; `PARTIAL` returns no Managed document and no whole-table projection; no public raw connection, FullSource/recovery/result/receipt/plan/raw-scope/ready-ledger input; provider authority or ledger mutation fails closed; public validator rejects reviewed and ledger input; v1, existing normalized projection, Canonical, facts, product and bundles stay unchanged | fake/mutable owner or validator injection, partial or synthetic source-unit identity, entry IDs presented as source atoms, public raw connection/scope/ledger bridge, caller-created adjudication result, second FullSource/recovery, caller status trust, provenance laundering, parallel parser/projection, product activation or bypass |
| Inactive sealed Managed v2 whole-table projection | private [`_project_sealed_adjudicated_managed_document`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/managed_whole_table_projection.py) | [Managed Document v2](./BROKER_REPORTS_MANAGED_DOCUMENT.v2.md) source-unit ledger and integrity, plus the same-call ledger plan derived from FullSource | the same adjudicated `ManagedPdfDocumentV2Builder` call only, immediately after private reviewed+ledger seal | representation-only deep copy; sealed payload must match the current FullSource-derived ledger plan; one COMPLETE Managed TABLE becomes one projection with exact rows, source parts, unit/atom/word coverage and Managed integrity; continuation headers remain source rows; typed `NOT_READY` is atomic; no public builder/input, Canonical, product or bundle activation | caller-supplied Managed payload/ledger/receipt, synthesized source IDs, row/header collapse or repair, second source resolver/validator, NormalizedTableProjection/Canonical/financial imports, partial publication or runtime activation |
