...
| Public dialogue presentation | plain-language wording of owner-produced public state and a non-authoritative answer proposal | `ordinary_trade_public_dialogue_context_v1` or one current public question plus the user's natural reply | validated `ordinary_trade_public_dialogue_message_v1` or a visible proposal which creates no fact until the user repeats the owner-accepted wording in a separate message; transport uses only the administrator-pinned HTTPS OpenWebUI origin, denies redirects and bounds response bytes | request/base-URL-derived destinations, cross-origin token forwarding, raw refs/status/reason/source rows, business decisions, direct Human Fact publication or silent interpretation | OpenWebUI Pipe, then the existing Human Fact owner for a later direct answer |
