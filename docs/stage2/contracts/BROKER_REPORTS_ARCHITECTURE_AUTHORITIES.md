...
| Gate 5 Human Fact Scope | build exact scope-bound Human requests, own immutable current-request publication, normalize authenticated answers, reject owner-visible conflicts and validate exact owner origin across runs in one case | authentication, case-to-taxpayer identity binding, tax/legal/source/external meaning, workflow state, declaration assembly or generic evidence | [`Gate5HumanGapClosureRuntimeFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/gate5_human_gap_closure.py), reusing `ArtifactResolver.resolve_case`; authenticated taxpayer-case binding remains an explicit missing upstream owner contract | [Gate 5 Human Fact Scope v1](./BROKER_REPORTS_GATE5_HUMAN_FACT_SCOPE.v1.md) | current supporting inactive Human publication/validation proof; product composition fails closed until taxpayer binding exists | case hash as taxpayer, `user_id` or operation subject as taxpayer, caller-published/current-selected requests, timestamp/ref authority, conflict omission, free-text external authority, v0 downgrade, provider/LLM calls or new registry/workflow/receipt engine |
