...
| Proposed/inactive Issue #317 document-wide visual adjudication | [`PdfDocumentVisualAdjudicationFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_document_visual_adjudication.py) orchestration only; existing raster, provider, binder and LogicalRow owners retain their meanings | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | focused coordinator controls only; no intake or product consumer | render every FullSource page once, exact same-raster PROPOSAL/CRITIC calls, deterministic page/observation/parser-candidate coverage ledger, exact-agreement binding and private raw same-call handoff | ready receipt input, model text/refs/table identity/continuation, visual-only promotion, ABSENT/EMPTY/EXPLAINER exclusion, retry/failover, Canonical/facts/product activation |
| Proposed/inactive Issue #317 source-bound table-scope receipt binder | [`SourceBoundTableScopeFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/source_bound_table_scope.py) representation-only | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | existing `LogicalRowTableFactory`, including the inactive Managed Document v2 same-call bridge; no product consumer | closed geometry-only proposal plus separate host envelope; whole FullSource validation and the existing locator coordinate owner bind exact candidate/word refs into a non-authoritative receipt | provider call/transport, ready-scope consumer input, receipt checksum as authority, model text/refs/table ID, continuation, candidate invention, authoritative exclusion, Canonical/facts/product activation |
