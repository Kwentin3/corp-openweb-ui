...
### GOAL 17 Type-First inactive additive profiles

The
[Type-First inactive implementation amendment v1](./BROKER_REPORTS_GATE2_TYPE_FIRST_INACTIVE_IMPLEMENTATION.v1.md)
implements the GOAL 16 semantic contract only through the existing operation
authorities below. These are additive methods and profile declarations, not
new authorities or product consumers.

| Concern | Existing sole owner | Additive Type-First symbol | Exact authority boundary |
| --- | --- | --- | --- |
| Three-field candidate and private local-to-canonical receipt | `Gate2FinancialSemanticV6PacketFactory` | `create_type_first_candidate` | Reuse exact current Context V2.1 `source` and `type_cards`; keep the active Packet and historical sidecars exact; mapping stays private and is never index-derived. |
| Managed type-card projection | `Gate2FinancialSemanticV5ProjectionFactory.create_minimal_managed_projection` | unchanged | No copied Pack wording, second projection or changed type boundary. |
| Response schema and parser | `Gate2FinancialSemanticV6ChoiceContractFactory` | `create_type_first_response_profile`; `normalize_financial_semantic_v6_type_first_response` | Parse only ordered `plausible_types`; reject non-object and duplicate-root responses technically; never sort, deduplicate or repair. |
| Complete request lint and sealing | `Gate2FinancialSemanticV6ContextLinterFactory` | `create_type_first` | Seal exact Prompt, candidate, Choice-owned schema and Packet-owned receipt; enforce the `2500`-byte logical-request ceiling. |
| Provider-neutral request construction | `Gate2OpenWebUIRequestBuilder` | `build_from_sealed_type_first` | The request profile is local-proof-only, requires the sealed entrypoint and stays outside production request profiles. |
| Provider schema binding and terminal extraction | `Gate2ProviderAdapterFactory.create` and `Gate2PreparedProviderRequest` | existing generic `prepare_form_data`, `validate_schema_binding` and `extract_prepared_content`; unchanged | The GOAL 17 support proof uses the existing OpenAI generic path with one simulated terminal envelope, then the Choice parser re-enforces the complete Type-First schema; no adapter profile, projection policy or transport is added. |
| Type-first decision expansion | `Gate2FinancialSemanticV6DecisionExpansionFactory` | `create_from_type_first_candidate` | Cardinality and exact complete-option count derive the canonical Choice; the model never returns a reason, option, record, value, ref or binding. |
| Canonical validation and materialization | `Gate2FinancialEvidenceValidatedDecisionFactory.create`; `Gate2FinancialEvidenceMaterializerFactory.create().materialize` | unchanged | Provider output remains a proposal; no alternate validator, ID minter, binding owner or materializer is allowed. |
| V6 terminal totality proof | `Gate2FinancialSemanticV6TotalMaterializerFactory` | unchanged | Historical totality remains exact; GOAL 17 calls the existing canonical validator/materializer directly and adds no Type-First totality entrypoint. |
| Financial Domain snapshot and persistence | `Gate2FinancialDomainCatalogFactory.create`; `Gate2FinancialDomainPersistenceFactory.serialize/restore` | unchanged | Successful local proof persists and restores one exact synthetic snapshot; technical failures create no snapshot. |
| Exact evidence, technical-failure evidence, replay and per-case comparator | `Gate2FinancialSemanticV6DecisionEvidenceFactory` | `create_type_first_candidate`; `create_type_first_technical_failure`; versioned restore/replay | Comparator consumes an independent test oracle only after the code-owned decision and cannot mutate it; private failure evidence retains exact invalid input, while safe evidence contains only allowlisted hashes, classes and counters. |
| Zero-call local proof composition | maintained GOAL 17 support builder/tests | compose existing owners only | The offline support entrypoint is not imported by product runtime and creates no coordinator, adapter, materializer or persistence authority. |
| Economy admission/accounting | `Gate2EconomyBudgetSessionFactory.create` | request-specific `broker_reports_gate2_type_first_one_call_no_fallback_v1` profile | Maximum calls per operation is one, maximum fallback calls is zero and GOAL 17 authorizes zero calls; historical workload policies stay unchanged. |
| Qualification profile declaration | `Gate2FinancialSemanticV6ContextV21BudgetSmokeCoordinator` | `type_first_profile_declaration` only | Declare identities, counters and hard gates only; GOAL 17 must not call `execute_slot`, create clients, consume slots or invoke transport. |

The Type-First profile has no product Pipe consumer and changes no feature
valve or production admission. Maintained request-builder and Economy source
bytes are deterministically re-embedded in all three closed-world generated
bundles, without changing their module/import topology or active Pipe
behavior. Provider calls, runtime activation and new authorities are all zero.
