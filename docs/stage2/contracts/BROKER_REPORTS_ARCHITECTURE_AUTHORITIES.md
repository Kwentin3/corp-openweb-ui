...
| Inactive Managed document semantic evidence | existing `ManagedPdfToCanonicalFactory.create_for_openwebui` same-call coordinator invokes the private evidence assembler in the existing `OrdinaryTradeSemanticMapping` owner before its Canonical result escapes | raw PDF bytes, source/request identity, Canonical version inputs and user scope only; caller cannot supply Canonical, Managed payload, projection, ledger, binding, evidence or receipt | `broker_reports_managed_document_semantic_evidence_v1`: the owner-produced Canonical's every table row/cell and ordered non-table literal once, opaque refs, host coverage ledger/hash, raw literals tagged `UNTRUSTED_SOURCE_DATA`, representation-only and consumer-ineligible | model response, critic, semantic disposition, ready receipt, provider call beyond the existing Managed PDF route, sampling/truncation, persistence, runtime or product activation | no consumer; a later separately reviewed slice may build proposal/critic input from this exact evidence hash |
| Inactive Managed semantic review contract | existing `ManagedPdfToCanonicalFactory.create_for_openwebui` same-call coordinator passes its private evidence plus raw proposal/critic JSON to the existing `OrdinaryTradeSemanticMapping` owner; compiler v4 validates selected mapping shapes | raw PDF/source/request identity, user scope, raw closed proposal response and raw closed critic response only; proposal exact-echoes an opaque evidence-scope ref, critic exact-echoes that ref plus the host proposal ref, and model fields otherwise remain opaque host refs and enums | content- and evidence-bound option refs, host-bound proposal options and critic selections with exact table coverage, receipt hashes and `REVIEWED_CANDIDATE` / `CLARIFICATION_REQUIRED` / `BLOCKED`; `UNRESOLVED` requires at least two distinct options, candidates always empty and `consumer_eligible=false` | caller Canonical/evidence/options/receipt, free text, question text, provider/model client, facts, runtime, persistence or product activation | no executable consumer; a later provider slice may supply the two raw responses without changing this authority boundary |
| Inactive Managed semantic provider coordinator | additive `ManagedPdfToCanonicalFactory.create_semantic_review_for_openwebui` route orders the existing raw-PDF/evidence owner, one Gate 2 proposal submission, host normalization, then one Gate 2 critic submission | raw PDF/source/request/user identity, provider profile and proposal/critic model IDs only; no caller client, endpoint, resolver, Canonical, evidence or raw response | the existing inactive semantic review contract plus technical two-call lifecycle/metadata accounting; no retry, fallback, repair, candidates, facts or publication | semantic classification, mapping validation, connection ownership, third call, question generation, runtime/store/Gate 4/product activation | no executable consumer |
| Inactive reviewed document candidate | the same raw-PDF provider coordinator passes only its same-call `REVIEWED_CANDIDATE` to the private existing `OrdinaryTradeSemanticMapping` owner, which reuses case qualification, compilation and document atomic assembly | exact selected `SECURITY_TRADES` options plus retained `SAFE_AUXILIARY` decisions bound to the same review/evidence/Canonical/user scope; callers cannot supply ready review, options, receipts, Canonical or evidence | an inactive atomic document candidate and inspectable same-call composition binding marked `consumer_eligible=false`, `runtime_activation=false` and `independent_derivation_proven=false`; any relevant partial table yields zero document record candidates, while safe auxiliary source remains inspectable | portable qualification/activation authority, facts, runtime/store/product activation, a third model call, fake compilation of safe tables, caller safe labels or user-confirmation claims; persistence or activation requires authoritative replay in a later versioned contract | no executable consumer |
...
| PDF table location | one box per visible table, native normalized-coordinate validation and deterministic projection to PDF points | source literals, row/cell inference, parser-setting selection, financial meaning or canonical publication | `PdfTableIntakeRuntimeFactory.create_for_openwebui` and `PdfTableLocatorProjectionFactory.create` | [PDF Source-Bound Table Normalization v1](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v1.md) | existing PDF layout/parser owner only | coordinate grids, model transcription, model-built pdfplumber plans or parallel canonical schemas |
...
| FullSource PDF layout char -> word -> line -> table-cell partition authority | existing [`FullSourceArtifactFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/full_source.py), [`PdfPlumberLayoutAdapter`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout.py) and [`PdfLayoutUnitBuilder`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout_units.py) only | `pdf_layout_source_chain_v0` / `pdfplumber_exact_char_identity_v0`; active partition policy `pdf_layout_unit_partition_policy_v3_unresolved_regions` | existing FullSource layout-unit consumers only; no new product route | while raw parser objects are still live, the FullSource owner exactly compares materialized chars, words, lines, unresolved regions and bboxes before minting active authority; a candidate word belongs to one cell only when every owned character center resolves to that same cell; addressed cells exactly cover the declared grid and their disjoint word-ref union exactly equals candidate contributing words; crossing, missing, foreign, duplicate, overlapping, ambiguous or out-of-bounds evidence blocks all table units atomically with no line fallback; exact wordless 1x1 cells remain representable; unbounded aligned-text evidence that has not proved a grid is preserved as an exact private unresolved region and blocks FullSource/Managed atomically instead of becoming `none`, `rejected`, a line fallback or partial facts; the payload validator checks serialized structural consistency only and never claims PDF-byte authenticity; pypdf text remains diagnostic; rotated text remains explicit `PARTIAL` | arbitrary-payload authority minting, self-recomputable source-authenticity receipt, pypdf substring as completeness authority, source-bound partial bypass, line fallback after a blocked or unresolved candidate, orientation completeness claim, Canonical/mapper/runtime repair, table identity/continuation, broker/header dictionaries, product activation or fact publication |

The `pdfplumber_layout_policy_v4_mixed_table_terminal` identity intentionally changes
derived refs and sealed hashes from v2. The bounded parity test freezes both the
old and new hashes while separately proving that table/row/word ownership counts
stay unchanged. No backward-reader promise is made without a serialized v2
artifact fixture.
...
| Proposed/inactive Issue #317 document-wide visual provider seam | existing [`PdfTableLocatorProviderFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_table_locator_provider.py) only | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | focused provider controls only; no intake or product consumer | one ordered bounded full-page image set, exact raster-owner identities, one exact-body `countTokens` preflight plus one native model generation, strict inline-byte/token guards, fixed capped page-array geometry/status schema and terminal trace; existing single-page APIs unchanged | coordinator, coverage/adjudication decision, provider retry/failover, model text/refs/table identity/continuation, intake activation, Canonical/facts/product mutation |
| Proposed/inactive Issue #317 document-wide visual adjudication | [`PdfDocumentVisualAdjudicationFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_document_visual_adjudication.py) orchestration only; existing raster, provider, binder and LogicalRow owners retain their meanings | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | focused coordinator controls only; no intake or product consumer | the additive D2 observation path runs only for D1 unresolved regions, renders every FullSource page once and makes exactly one ordered PROPOSAL generation plus its exact-body token preflight; it binds geometry through the existing source owner and always remains BLOCKED/non-authoritative; the historical two-pass recovery proof stays compatibility-only | ready receipt input, model text/refs/grid/cells/financial roles/table identity/continuation, visual-only promotion, ABSENT/EMPTY/EXPLAINER exclusion, retry/critic/chunk/failover in D2, Canonical/facts/product activation |
| Proposed/inactive Issue #317 source-bound table-scope receipt binder | [`SourceBoundTableScopeFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/source_bound_table_scope.py) representation-only | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | existing `LogicalRowTableFactory` legacy proof and the inactive D2 coordinator only; no product consumer | closed geometry-only proposal plus separate host envelope; whole FullSource validation and the existing locator coordinate owner bind exact candidate refs for the legacy proof or D1 unresolved refs for D2; D2 promotes a word into section/title/header/body evidence only when every owned character center is inside the role box, preserves source order and rejects overlapping roles or ambiguous region coverage | provider call/transport, ready-scope consumer input, receipt checksum as authority, model text/refs/table ID, continuation, candidate invention, authoritative exclusion, Canonical/facts/product activation |
| Proposed/inactive Issue #317 logical-table continuation owner | existing [`LogicalRowTableFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/logical_row_table_recovery.py) only | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | document-wide adjudicator and focused owner/corpus controls only; no product consumer | factory-routed same-call binding consumes only adjudicator-owned original proposals plus whole FullSource/host evidence; receipts stay private; full stable header evidence, title boundary, unique predecessor and inspectable ambiguity; v1 `recover` remains unchanged | public ready-receipt/scope input, product import or activation, competing continuation writer, model-authored source values, dictionary/regex route, Canonical repair or fact publication |
| Inactive PDF -> Managed Document v2 orchestration | [`ManagedPdfDocumentV2Factory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/managed_pdf_document_v2.py) | [Managed Document v2](./BROKER_REPORTS_MANAGED_DOCUMENT.v2.md); legacy `create(schema).build(...)` remains unchanged, while `create_adjudicated_for_openwebui(schema, request)` resolves the enabled Admin provider through the existing OpenWebUI connection owner before returning a builder whose public build accepts only original PDF bytes, private source identity, task ID and DPI | offline DOC6 recovery proof, adjudicated row-oriented view and inactive whole-table projection diagnostics only | exact FullSource runs once; the exact factory-created document adjudicator alone owns admission; its same-call recovery reaches private Managed assembly only on `COVERAGE_COMPLETE`; that same owner binds each table part to exact whole FullSource units and seals real unit/checksum/atom/word coverage; the public result does not expose ready FullSource payload/unit dictionaries for Canonical; `PARTIAL` returns no Managed document and no whole-table projection; no public raw connection, FullSource/recovery/result/receipt/plan/raw-scope/ready-ledger input; provider authority or ledger mutation fails closed; public validator rejects reviewed and ledger input; v1, existing normalized projection, facts and product stay unchanged | fake/mutable owner or validator injection, partial or synthetic source-unit identity, entry IDs presented as source atoms, public raw connection/scope/ledger bridge, caller-created adjudication result, second FullSource/recovery, caller status trust, provenance laundering, parallel parser/projection, product activation or bypass |
| Inactive sealed Managed v2 whole-table projection | private [`_project_sealed_adjudicated_managed_document`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/managed_whole_table_projection.py) | [Managed Document v2](./BROKER_REPORTS_MANAGED_DOCUMENT.v2.md) source-unit ledger and integrity, plus the same-call ledger plan derived from FullSource | the same adjudicated `ManagedPdfDocumentV2Builder` call only, immediately after private reviewed+ledger seal | representation-only deep copy; sealed payload must match the current FullSource-derived ledger plan; one COMPLETE Managed TABLE becomes one projection with exact rows, source parts, unit/atom/word coverage and Managed integrity; continuation headers remain source rows; typed `NOT_READY` is atomic; public projection output is diagnostic, not Canonical authority; no public builder/input, product or bundle activation | caller-supplied Managed payload/ledger/receipt, synthesized source IDs, row/header collapse or repair, second source resolver/validator, NormalizedTableProjection/financial imports, partial publication or runtime activation |
| Inactive Managed PDF v2 -> Canonical coordinator | [`ManagedPdfToCanonicalFactory.create_for_openwebui`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/managed_pdf_to_canonical.py) composing `ManagedPdfDocumentV2Factory` and `CanonicalNormalizerFactory` | [Managed Document v2](./BROKER_REPORTS_MANAGED_DOCUMENT.v2.md), [Canonical Artifact v1](./BROKER_REPORTS_CANONICAL_ARTIFACT.v1.md) and proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | inactive same-call proof route only; no product consumer | accepts only original PDF bytes, tenant/version/source identity, task ID and DPI; obtains FullSource payloads/units through a private same-call Managed handoff; Canonical only represents owner-issued complete tables after checking source ref/checksum and exact equality of projection rows, roles, anchors, source parts and coverage against the sealed Managed payload; no facts are published | caller-supplied source payloads, source units, Managed payloads, table projections, source-ref rebinding, projection self-hash as authority, Canonical table recovery, mapper repair, store writes, product activation or partial fact publication |
