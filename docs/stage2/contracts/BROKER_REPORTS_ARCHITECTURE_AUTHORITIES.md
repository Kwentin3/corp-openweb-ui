...
| Public dialogue presentation | plain-language wording of owner-produced public state and a non-authoritative free-answer proposal | `broker_reports_ordinary_trade_public_dialogue_context_v2` plus the current user's natural reply; mapping clarification enters only through the bounded `MAPPING_CLARIFICATION` action with an opaque current question binding, code-validated option bindings and source literals explicitly tagged as untrusted quoted data | validated `broker_reports_ordinary_trade_public_dialogue_message_v2` or `broker_reports_ordinary_trade_public_interpretation_v1`; for mapping clarification v2 permits one LLM-authored natural question bound to the exact current question and complete option-ref set, keeps source-derived lexical surface inside quoted evidence, and rejects a missing/foreign binding, an added speech act or source-taint escape before using the deterministic fallback; mapping confirmation remains exact and code-owned; a candidate creates no fact before native confirmation; Pipe transport uses only the administrator-pinned HTTPS OpenWebUI origin, denies redirects and bounds response bytes | model-supplied contract identity, foreign/missing question or option binding, source-derived text outside quoted evidence, an additional action/data request/financial meaning, model-authored confirmation, request/base-URL-derived destinations, cross-origin token forwarding, raw mapping state, internal role codes, Fact contract names, raw refs/status/reason/source rows, business decisions, request identity, direct Human Fact publication, silent interpretation or more than one presentation call per conversation turn | OpenWebUI Pipe, explicit user confirmation, then the existing Human Fact or mapping case owner |
