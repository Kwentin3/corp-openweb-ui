...
| FullSource PDF layout char -> word -> line partition authority | existing [`FullSourceArtifactFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/full_source.py), [`PdfPlumberLayoutAdapter`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout.py) and [`PdfLayoutUnitBuilder`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout_units.py) only | `pdf_layout_source_chain_v0` / `pdfplumber_exact_char_identity_v0`, validated by the existing PDF payload validator; active partition policy `pdf_layout_unit_partition_policy_v1_source_chain` | existing FullSource layout-unit consumers only; no new product route | validated exact pdfplumber char -> word -> line ownership controls layout completeness and unit partition; pypdf page text and substring comparison remain compatibility diagnostics only; no-table, rejected, blocked and candidate table terminals are distinct; rotated text remains explicit `PARTIAL` | pypdf substring as completeness authority, source-bound partial bypass, orientation completeness claim, Canonical/mapper/runtime repair, table identity/continuation, broker/header dictionaries, product activation or fact publication |

The `pdfplumber_layout_policy_v3_table_terminals` identity intentionally changes
