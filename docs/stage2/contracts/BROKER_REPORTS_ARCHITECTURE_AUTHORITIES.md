...
| Gate 5 Human Fact Scope | build exact scope-bound Human requests, publish request/fact artifacts through the existing ArtifactStore, normalize authenticated answers and validate exact owner origin across runs in one case | authentication, taxpayer identity truth, tax/legal/source/external meaning, workflow state, declaration assembly or generic evidence | [`Gate5HumanGapClosureRuntimeFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/gate5_human_gap_closure.py), reusing `ArtifactResolver.resolve_case`; `gate5_case_taxpayer_scope_ref` owns only the opaque bounded case-local taxpayer slot | [Gate 5 Human Fact Scope v1](./BROKER_REPORTS_GATE5_HUMAN_FACT_SCOPE.v1.md) | current supporting Human publication/validation boundary; declaration assembly remains inactive | `user_id` as taxpayer, operation subject as taxpayer, caller-published requests, hash-only origin, v0 downgrade, run-bound facts, last-write-wins, provider/LLM calls, source/external/tax authority or new registry/workflow/receipt engine |
