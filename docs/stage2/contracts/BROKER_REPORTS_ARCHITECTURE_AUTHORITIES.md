...
-> independent same-evidence critic for every model proposal
...
| --- | --- | --- | --- | --- | --- |
| Source intake and normalization | source bytes, current-page table/title/header location, adjacent-page whole-table identity, deterministic pdfplumber structure/literals, one pre-Canonical logical-table assembly and provenance | authenticated upload and one bounded visual response per page; after page 1 the same call sees previous and current original full-page PNGs | persisted Gate 1 artifacts and validated complete Canonical candidate | financial labels, row values from the model, tax meaning, broker/header dictionaries, Canonical or mapper table repair | Canonical lifecycle |
...
| Case semantic mapping | strict source-semantic proposal plus independent critic for every Canonical table not covered by the frozen exact-schema fast path; bounded clarification options carry validator-checked machine decisions | one code-built Canonical title + flattened header + ordered body view per exact unknown table node, with opaque table refs; proposal and critic receive byte-identical case evidence through `OrdinaryTradeSemanticMappingFactory.create` | only the critic-reviewed strict mapping response or one clarification; runtime binds the Canonical header count and internal Canonical/case IDs and hashes stay code-only | model-selected physical header row, remapping a frozen table, Canonical mutation, broker/year/filename routing, fuzzy admission, retry, repair, partial publication, truncated-evidence `NO_NAMED_CONSUMER` or unreviewed table exclusion | append-only mapping case |
| Mapping case | exact user/case/chat/workspace and Canonical binding, same-case resume, stale-answer rejection and native explicit confirmation | independently reviewed proposal or interpreted answer | new revisions use `broker_reports_ordinary_trade_mapping_case_v3` and seal the semantic-evidence digest; frozen v2 artifacts remain read-compatible and an in-progress v2 revision continues as a v3 successor with predecessor continuity; a v2 `COMPLETE` remains legacy-qualified without a retroactive critic claim; code renders public options/confirmation from the validated decision; only `COMPLETE` exposes case-qualified material after confirmed-decision checks, full side-literal coverage and a zero-unmapped deterministic dry-run | implicit confirmation, model-authored public decision wording, label-only confirmation, sample-only completeness, overwrite, cross-tenant reuse, global registry promotion or describing a v2 case as independently reviewed | qualified mapping authority |
| Qualified mapping authority | meaning of one exact table schema, literal side enum and amount-column to currency-column bindings; frozen receipt v2 remains the global zero-call schema fast path and case receipt v1 is executable only at its exact authenticated `table_node_id`; a `retained_transaction_charge` relation names `broker_reports_source_observation_v1.retained_amount_currency`, never the Fact consumer | package registry or confirmed mapping case | mapping v3 plus `broker_reports_ordinary_trade_mapping_qualification_v2` or `broker_reports_ordinary_trade_case_mapping_qualification_v1` | row/value authorship, broker/year/filename profile keys, fuzzy matching, unconfirmed admission, unsupported-amount laundering or reuse of a case mapping outside its exact table node | semantic compiler through the projection owner only |
| Ordinary-trade semantic compiler | exact schema match per independent table node, deterministic binding of the flattened Canonical header to its source-bound header-row count, exact execution of case-scoped mappings, explicit display-only no-consumer retention, named-financial-value blocking, qualified amount/currency bindings, SourceObservation-only retention of source-stated transaction charges with no current Fact consumer, deterministic date/decimal transforms and runtime-record lineage | active Canonical plus global frozen mappings and separately scoped case mappings; physical values come only from immutable `content.cells` below the bound header count | `broker_reports_ordinary_trade_runtime_projection_v5`: Source Observations, qualified authority lineage, mapping-case ref and runtime records; retained-charge refs remain outside records/claims/Facts | header repair or model-selected header rows, registry/case overlap on one node, tax, proximity/adjacency binding, unsupported-meaning laundering, partial publication from an incomplete row, relations, inferred continuation, value deduplication, Canonical mutation | projection store and Gate 4 adapter |
