...
| FullSource PDF layout char -> word -> line shadow receipt | existing [`FullSourceArtifactFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/full_source.py), [`PdfPlumberLayoutAdapter`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout.py) and [`PdfLayoutUnitBuilder`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout_units.py) only | `pdf_layout_source_chain_v0` / `pdfplumber_exact_char_identity_v0`, validated by the existing PDF payload validator | FullSource diagnostics only; no admission or product consumer | exact pdfplumber returned-character identity becomes page-scoped char refs, word ownership and exact line ownership; blanks and duplicate overlays have explicit dispositions; receipt/status/reasons/checksums fail rotated text explicitly; active layout completeness and table admission stay unchanged | substring or geometry rebinding, orientation completeness claim, Canonical/mapper/runtime repair, table identity/continuation, broker/header dictionaries, product activation or fact publication |
