...
| FullSource PDF layout char -> word -> line partition authority | existing [`FullSourceArtifactFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/full_source.py), [`PdfPlumberLayoutAdapter`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout.py) and [`PdfLayoutUnitBuilder`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/pdf_layout_units.py) only | `pdf_layout_source_chain_v0` / `pdfplumber_exact_char_identity_v0`; active partition policy `pdf_layout_unit_partition_policy_v1_source_chain` | existing FullSource layout-unit consumers only; no new product route | while raw parser objects are still live, the FullSource owner exactly compares materialized chars, words, lines and bboxes before minting active authority; the payload validator checks serialized structural consistency only and never claims PDF-byte authenticity; pypdf text remains diagnostic; no-table, rejected, blocked, candidate and candidate-with-rejections terminals are distinct; mixed/rejected/blocked table admission is atomic; rotated text remains explicit `PARTIAL` | arbitrary-payload authority minting, self-recomputable source-authenticity receipt, pypdf substring as completeness authority, source-bound partial bypass, orientation completeness claim, Canonical/mapper/runtime repair, table identity/continuation, broker/header dictionaries, product activation or fact publication |
