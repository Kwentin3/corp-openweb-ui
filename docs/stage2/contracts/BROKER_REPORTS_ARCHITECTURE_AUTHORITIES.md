...
| Ordinary-trade semantic compiler | exact schema match per independent table node, deterministic binding of the flattened Canonical header to its source-bound header-row count, exact execution of case-scoped mappings, explicit display-only no-consumer retention, named-financial-value blocking, qualified amount/currency bindings, deterministic date/decimal transforms and runtime-record lineage | active Canonical plus global frozen mappings and separately scoped case mappings; physical values come only from immutable `content.cells` below the bound header count | `broker_reports_ordinary_trade_runtime_projection_v5`: Source Observations, qualified authority lineage, mapping-case ref and runtime records | header repair or model-selected header rows, registry/case overlap on one node, tax, proximity/adjacency binding, relations, inferred continuation, value deduplication, Canonical mutation | projection store and Gate 4 adapter |
