...
-> independent same-Canonical semantic review of risky mapper terminals
-> append-only irreducible-clarification/confirmation case + qualification receipt
-> Source Observations
...
| Case semantic mapping | strict source-semantic proposal for every Canonical table not covered by the frozen exact-schema fast path, followed by one independent document-wide review of every proposed `COMPLETE` or `CLARIFICATION_REQUIRED` terminal against the exact same Canonical package; a disputed `COMPLETE` receives one bounded same-evidence adjudication and no retry | bounded Canonical value sample and full distinct-value surface for exact unknown table nodes with opaque table refs through `OrdinaryTradeSemanticMappingFactory.create`; reviewer and adjudicator receive only the machine proposal plus the same evidence, and adjudication also receives the first review | reviewed complete, adjudicated complete, reviewer-selected obvious interpretation, typed unsafe rejection, or one reviewer-certified irreducible clarification; review receipt v1 binds two calls, while `broker_reports_ordinary_trade_semantic_adjudication_receipt_v2` binds all three calls and Canonical root | remapping a frozen table, Canonical mutation, broker/year/filename/header-dictionary routing, mapper-authored safe exclusion, compilation-as-ambiguity proof, fuzzy admission, retry, repair, partial publication or mandatory confirmation of every table | append-only mapping case |
| Mapping case | exact user/case/chat/workspace and Canonical binding, same-case resume, stale-answer rejection and native explicit confirmation for irreducible ambiguity | independently reviewed mapper proposal or interpreted answer | `broker_reports_ordinary_trade_mapping_case_v4`; only review verdict `APPROVE_COMPLETE`/`SELECT_OPTION` permits `COMPLETE`, `IRREDUCIBLE_AMBIGUITY` permits one question and `REJECT_UNSAFE` persists the fail-closed terminal; code renders public options/confirmation from the validated decision | unreviewed completion/clarification, implicit confirmation, model-authored public decision wording, label-only confirmation, sample-only completeness, overwrite, cross-tenant reuse, global registry promotion | qualified mapping authority |
...
| Ordinary-trade semantic compiler | exact schema match per independent table node, exact execution of case-scoped mappings, explicit display-only no-consumer retention, named-financial-value blocking, qualified amount/currency bindings, deterministic date/decimal transforms and runtime-record lineage | active Canonical plus global frozen mappings and separately scoped case mappings | `broker_reports_ordinary_trade_runtime_projection_v6`: Source Observations, qualified authority lineage, mapping-case ref and runtime records | registry/case overlap on one node, tax, proximity/adjacency binding, relations, inferred continuation, value deduplication, Canonical mutation | projection store and Gate 4 adapter |
