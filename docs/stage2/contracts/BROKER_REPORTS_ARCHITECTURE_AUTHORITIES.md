...
| Proposed/inactive Issue #317 logical-table continuation owner | existing [`LogicalRowTableFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/logical_row_table_recovery.py) only | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | focused owner and corpus controls only; no product consumer | full stable header evidence, source-bound title boundary, unique predecessor and inspectable ambiguity; current v1 route remains unchanged | product import or activation, competing continuation writer, model-authored source values, dictionary/regex route, Canonical repair or fact publication |
