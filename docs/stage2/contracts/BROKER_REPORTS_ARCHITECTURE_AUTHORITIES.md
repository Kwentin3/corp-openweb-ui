...
| Proposed/inactive Issue #317 logical-table continuation owner | existing [`LogicalRowTableFactory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/logical_row_table_recovery.py) only | proposed/inactive [PDF Source-Bound Table Normalization v2](./BROKER_REPORTS_PDF_SOURCE_BOUND_TABLE_NORMALIZATION.v2.md) | existing inactive `ManagedPdfDocumentV2Factory` and focused owner/corpus controls only; no product consumer | factory-routed same-call binding consumes only original proposals plus whole FullSource/host evidence; receipts stay private; full stable header evidence, title boundary, unique predecessor and inspectable ambiguity; one model-authored `ABSENT` stays non-authoritative pending an independent later critic; v1 `recover` remains unchanged | public ready-receipt/scope input, product import or activation, competing continuation writer, model-authored source values, dictionary/regex route, Canonical repair or fact publication |
| Inactive PDF -> Managed Document v2 orchestration | [`ManagedPdfDocumentV2Factory`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/managed_pdf_document_v2.py) | [Managed Document v2](./BROKER_REPORTS_MANAGED_DOCUMENT.v2.md); legacy `build` and additive `build_with_source_bound_scopes` accept original PDF bytes and each invokes FullSource exactly once; only the additive method accepts raw scope requests | offline DOC6 recovery proof and row-oriented view only | factory/builder expose no owner or validator DI/mutable attrs; each private build call constructs exact owners and validator as locals; no public FullSource/ready-evidence input; legacy rejects all source-bound/reviewed output; scoped binding promotes only accepted `PRESENT` leading structure, while `ABSENT`/non-data/partial receipts remain audit-only with no reviewed plan; public validator rejects reviewed input; v1, product and bundles stay unchanged | fake/mutable owner or validator injection, `BOUND` receipt treated as authority, ready receipt/evidence input, public FullSource result, caller status trust, provenance laundering, parallel parser/projection, direct LogicalRow consumer, product activation or bypass |
