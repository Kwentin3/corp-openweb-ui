...
| Case semantic mapping | strict source-semantic proposal for every Canonical table not covered by the frozen exact-schema fast path, followed by one independent document-wide review of every proposed `COMPLETE` or `CLARIFICATION_REQUIRED` terminal against the exact same Canonical package | bounded Canonical value sample and full distinct-value surface for exact unknown table nodes with opaque table refs through `OrdinaryTradeSemanticMappingFactory.create`; reviewer receives only the machine proposal plus the same evidence | reviewed complete, reviewer-selected obvious interpretation, typed unsafe rejection, or one reviewer-certified irreducible clarification; `broker_reports_ordinary_trade_semantic_review_receipt_v1` binds both calls and Canonical root | remapping a frozen table, Canonical mutation, broker/year/filename/header-dictionary routing, mapper-authored safe exclusion, compilation-as-ambiguity proof, fuzzy admission, retry, repair, partial publication or mandatory confirmation of every table | append-only mapping case |
