...
-> frozen exact mapping fast path OR strict proposal plus independent critic
...
| Case semantic mapping | one strict source-semantic proposal and one independent critic for every Canonical table not covered by the frozen exact-schema fast path; the critic sees the same evidence and may approve, replace, retain one genuine ambiguity or reject unsafe loss | bounded Canonical value sample for exact unknown table nodes with opaque table refs through `OrdinaryTradeSemanticMappingFactory.create` | only the critic-reviewed mapping response or one clarification; both executions are bound into the case qualification hashes while internal Canonical/case IDs stay code-only | remapping a frozen table, Canonical mutation, broker/year/filename routing, fuzzy admission, retry, repair, partial publication, persisting an unreviewed proposal or excluding possible financial rows | append-only mapping case |
