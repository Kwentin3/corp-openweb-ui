...
| Exact profile composition | `Gate5TrustedMethodologyAuthorityFactory.create` owns the tax product rules; `Gate5FullTargetXmlProjectionDefinitionAuthorityFactory.create` owns form/KND/order/format/XSD target identity | selected Human Fact year plus exact source assertions | supported profile only when the selected year and both owner-produced identities agree; otherwise a typed unsupported-profile result | silent year substitution, closest-profile fallback, copied form/XSD constants | declaration adapter |
