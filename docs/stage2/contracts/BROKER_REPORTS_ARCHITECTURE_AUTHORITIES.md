...
| Managed model-facing asset-family identity and composition | additive [`broker_reports_financial_domain_assets.v2.manifest.json`](../../../services/broker-reports-gate1-proof/managed_assets/broker_reports_financial_domain_assets.v2.manifest.json), immutable v1 predecessor, their one deterministic builder and single closed-world `load_gate2_financial_semantic_model_assets` entrypoint | [OpenWebUI Financial Domain Asset Family v2](./BROKER_REPORTS_OPENWEBUI_FINANCIAL_DOMAIN_ASSET_FAMILY.v2.md) and the additive [Outcome Taxonomy v1](./BROKER_REPORTS_GATE2_OUTCOME_TAXONOMY.v1.md) | active v1 consumers plus the non-active Context V2.0 asset projection; GOAL 6 catalog v2 is an unpackaged inactive successor, and GOAL 7 owns its future minimal projection/family integration | family v2 and Context V2.0 remain immutable historical drafts; GOAL 7 must integrate the selected catalog v2 minimal projection through this same family/loader route without changing full asset bytes | parallel asset-family/manifest authority, in-place family v2 rewrite, second catalog loader, financial-semantic registry, custom asset GUI or adapter-owned semantic text |
| Human decision-reason meaning | immutable historical [`broker_reports_gate2_financial_decision_reason_catalog.v1.json`](../../../services/broker-reports-gate1-proof/managed_assets/decision_reasons/broker_reports_gate2_financial_decision_reason_catalog.v1.json) plus additive inactive [`broker_reports_gate2_financial_decision_reason_catalog.v2.json`](../../../services/broker-reports-gate1-proof/managed_assets/decision_reasons/broker_reports_gate2_financial_decision_reason_catalog.v2.json), each under the same catalog ID and versioned validator boundary | [Financial Decision Reason Catalog v1](./BROKER_REPORTS_GATE2_FINANCIAL_DECISION_REASON_CATALOG.v1.md) and [Outcome Taxonomy v1](./BROKER_REPORTS_GATE2_OUTCOME_TAXONOMY.v1.md) | v1 remains the implemented non-active [Context V2.0 candidate](./BROKER_REPORTS_GATE2_LLM_SEMANTIC_CONTEXT.v2.md); v2 adds the future count-one meaning for GOAL 7 minimal projection but has no active response consumer | active code set remains in the V6 decision/Choice contracts; catalog v2 is `runtime_activation=false`, unpackaged until GOAL 7 and ineligible for transport until a separately authorized V2.1 Choice profile exists | human wording in Python, Prompt, packet, adapter, report projector, Pack, an in-place v1 edit or a second active catalog/loader |
| Model-visible semantic context | [`Gate2FinancialSemanticV6PacketFactory.create`](../../../services/broker-reports-gate1-proof/broker_reports_gate1/gate2_financial_semantic_v6_packet.py) | implemented historical [LLM Semantic Context v1](./BROKER_REPORTS_GATE2_LLM_SEMANTIC_CONTEXT.v1.md) candidate, implemented non-active [Context V2.0 completeness baseline plus packet-owned private mapping receipt](./BROKER_REPORTS_GATE2_LLM_SEMANTIC_CONTEXT.v2.md), future [Minimal Model Surface](./BROKER_REPORTS_GATE2_MINIMAL_MODEL_SURFACE.v1.md), [current V6 Packet](./BROKER_REPORTS_GATE2_FINANCIAL_SEMANTIC_PACKET_V6.md) | current request builder still consumes only the active packet payload; GOAL 5 is docs-only, GOAL 7 implements its exact managed mappings, GOAL 8 owns only the non-active V2.1 candidate plus private mapping receipt | Context V1/V2.0 candidates are non-active; GOAL 8 creates no V2.1 Choice schema; current four-block V6 packet remains active and byte-frozen | second packet/context builder, unallowlisted model-visible field or provider-side semantic context rewrite |
...
6. after that STOP is cleared, GOAL 9 adds only the V2.1 linter/budget guard
   and consumes the P01-P18 Prompt, Packet candidate and Choice-owned schema;
   it does not invent or build a Choice schema.
...
Catalog v2 is an additive successor under the existing catalog ID. It is
inactive, is not packaged into immutable family v2, and is not accepted by
the active V6 Choice. GOAL 7 may implement only its managed minimal projection
through the existing loader/projection owner. The separately authorized V2.1
Choice-profile STOP before GOAL 9 remains unchanged.
