...
```text
ru-3ndfl-2025-declaration-input-contract@2026.0-audited
```
...
| foreign-source income and foreign tax | same source component owner | Articles 208, 214 and 232 | `SOURCE`, `CONTRACT` or reviewed treaty gap |
| securities and derivatives results | `Gate5DeclarationFinancialInvestmentResultsRuntimeFactory.create` | source-fact `2026.5`, operation `2026.1`, Article 214.1 input rules | exact source or methodology gap |
...
Gap classes are closed to `SOURCE`, `INTAKE`, `CONTRACT`, `METHODOLOGY` and
`USER/CASE`.

- `SOURCE`: missing acquisition quantity/roles, invalid source date or decimal,
  and absent authoritative payer/jurisdiction/tax documents.
- `CONTRACT`: no current normalized issuer/payer, realization-location,
  organized-market/IIS, CBR-rate or Article 232 document DTO for the supplied
  corpus.
- `METHODOLOGY`: partial-acquisition commission allocation, non-RUB
  intermediate precision/rounding and treaty-specific credit limits remain
  explicitly unresolved; no allocation, conversion or credit is emitted.
- `USER/CASE`: filing instance, taxpayer confirmation plus factual residence
  inputs, signer/authority and budget intent.
