...
qualified mapping-v3 pair of exact source columns. A matching immutable
qualification receipt must name its consumer and cover every such pair. The
compiler executes the pair and may not reconstruct it from adjacency,
uniqueness, row values or a successful downstream result.
...
| Ordinary-trade qualified source semantics | exact active Canonical plus immutable exact schema/enum/amount-currency mapping v3 and matching qualification receipt v2 | table-schema meaning, row disposition, source-bound roles, receipt-covered amount-column to currency-column binding and deterministic syntax-only date/decimal transforms | every non-empty data row after a matched header, every non-empty unknown-table row, exact literals/cell/provenance refs, mapping and qualification identities and unmapped disposition; titles/headers remain in Canonical/mapping evidence | broker/year/filename profile routing, fuzzy match, unqualified or unauditable relation admission, adjacency/proximity binding, table continuation, economic relations, reconciliation, tax or missing values | immutable Source Observations and deterministic runtime records | Gate 4 ordinary adapter | unknown/incomplete/unbound/unqualified remains `RELEVANT_UNMAPPED` or fails admission; ambiguous input fails closed; no fallback |
