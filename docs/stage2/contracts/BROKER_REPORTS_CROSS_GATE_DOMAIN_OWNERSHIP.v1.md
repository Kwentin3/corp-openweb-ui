...
| Gate 5 human/document closure | `Gate5HumanGapClosureRuntimeFactory.create` | formal findings, active demands, trusted `ArtifactAccessContext`, one bounded taxpayer slot and tax period | owner-published exact requests plus private user/case/taxpayer/period/request-bound v1 facts; same-case cross-run validation | caller-authored request publication, raw-transaction dialog, user-authored tax conclusions, source/external facts or blocker closure by LLM |
