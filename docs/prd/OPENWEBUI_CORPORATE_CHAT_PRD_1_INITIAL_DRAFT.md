...
Ключевая идея: сотрудники должны не просто заходить в чат и каждый раз заново объяснять задачу модели, а работать в подготовленных рабочих сценариях, где есть правила, шаблоны, промты, модели, документы, ограничения и понятная usage/cost visibility.
...
2. Web-search через иностранный и российский provider.
...
3. Если нужны hard budgets, rate limits, virtual keys, team budgets, centralized model routing и единая usage/cost policy по providers - изучать LiteLLM отдельным gateway-slice.
...
Цель - управляемый web-search с usage/cost visibility, источниками и доступом.
...
- финансовые условия provider path проверяются вне GitHub-документации;
...
- добавить ограничения и внешнюю ссылку на provider billing docs без денежных сумм;
...
- есть ограничения и внешняя ссылка на provider billing docs без денежных сумм;
...
- зафиксировать usage/cost calculation approach без денежных сумм в GitHub;
- хранить provider catalog в docs без денежных сумм;
...
## 11. Provider Commercial Terms Policy
...
| Brave Search API - Answers | Grounded answers | answers plan, request/token units, capacity limits | https://brave.com/search/api/ | Не путать с обычным Search. |
...
| YandexGPT Lite/Pro | Российский LLM | input/output token units, region and quota terms | https://aistudio.yandex.ru/docs/en/ai-studio/pricing.html | Требует quality smoke. |
| GigaChat API legal PAYG | Российский LLM | token units, legal-entity terms, auth/contracts | https://developers.sber.ru/docs/ru/gigachat/tariffs/legal-tariffs | Для юрлиц. |
| Claude classes | LLM economy/balanced/premium | input/output token units and context limits | https://platform.claude.com/docs/en/about-claude/models/overview | Проверять по quality needs. |
| OpenAI model classes | LLM primary/stronger/premium | input/output token units, context and tool terms | https://openai.com/api/pricing/ | Выбирать по quality smoke and policy. |
| OpenAI STT | Transcription fallback | audio minute unit and model availability | https://developers.openai.com/api/docs/pricing | Альтернатива Lemonfox. |
| OpenAI Web Search tool | Web-search alternative | call/token units and tool terms | https://developers.openai.com/api/docs/pricing | Не основной путь для OpenWebUI web-search. |
...
| Lemonfox STT | Speech-to-text | audio-hour unit, upload limits and region terms | https://www.lemonfox.ai/apis/speech-to-text | Provider path проверяется через server-side boundary. |
