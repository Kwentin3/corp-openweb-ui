...
правилами, доступами, ограничениями и понятной usage/cost visibility.
...
1. Сначала использовать нативные возможности OpenWebUI: Analytics, token usage, per-user/per-model
   ...
   budgets.
2. Если нужно только видеть примерный usage/cost signal - gateway не обязателен.
3. Если нужны hard budgets, rate limits, virtual keys, team budgets, centralized routing и единая
   usage/cost policy по providers - изучать LiteLLM отдельным gateway-slice.
...
- начать с `brave_llm_context` как иностранного provider;
...
Это оценка трудозатрат на реализацию, а не финансовые условия LLM/API, web-search, STT, сервера или хранения.
Финансовые условия фиксируются вне GitHub-документации. После Stage 2A диапазоны нужно уточнить по фактической версии OpenWebUI, доступам,
...
- разделить правила для иностранных и отечественных provider;
...
## 13. Provider Commercial Terms Policy

Денежные суммы и provider тарифы не хранятся в GitHub markdown. Перед коммерческим предложением,
production-включением или закупкой нужно перепроверить внешние billing pages и зафиксировать
финансовые условия вне репозитория.

| Сервис | Назначение | Что проверять вне GitHub | Источник | Комментарий |
| --- | --- | --- | --- | --- |
| Brave Search API - Search | Иностранный web-search | Search/LLM-context plan, request unit, credits/limits | https://brave.com/search/api/ | Хороший первый кандидат для web-search. |
| Brave Search API - Answers | Grounded answers | answers plan, request/token units, capacity limits | https://brave.com/search/api/ | Не путать с обычным Search. Для PRD-1 лучше начать с `brave_llm_context`. |
| Yandex Search API | Российский web-search | sync/deferred/generative modes, quota, metadata forwarding | https://aistudio.yandex.ru/docs/en/search-api/pricing.html | Проверить native Yandex provider или внешний adapter. |
| YandexGPT Lite | Российский LLM | input/output token units, contract region, quota | https://aistudio.yandex.ru/docs/en/ai-studio/pricing.html | Кандидат для массовых русских задач, требует quality smoke. |
| YandexGPT Pro 5.1 | Российский LLM | input/output token units, cached/tool token policy | https://aistudio.yandex.ru/docs/en/ai-studio/pricing.html | Проверить качество для документов и делового русского. |
| GigaChat API Lite legal PAYG | Российский LLM | token units, legal-entity terms, minimum terms | https://developers.sber.ru/docs/ru/gigachat/tariffs/legal-tariffs | Для юрлиц. Старые модели перенаправляются на GigaChat-2 equivalents. |
| GigaChat API Pro legal PAYG | Российский LLM | token units, legal-entity terms, auth/contracts | https://developers.sber.ru/docs/ru/gigachat/tariffs/legal-tariffs | Проверить auth, contracts, commercial terms. |
| GigaChat API Max legal PAYG | Российский LLM | token units, legal-entity terms, limits | https://developers.sber.ru/docs/ru/gigachat/tariffs/legal-tariffs | Premium RU path after pilot. |
| Claude Haiku | LLM economy | input/output token units and context limits | https://platform.claude.com/docs/en/about-claude/models/overview | Быстрая Claude-модель. |
| Claude Sonnet | LLM balanced | input/output token units and context limits | https://platform.claude.com/docs/en/about-claude/models/overview | Основной Claude candidate по quality. |
| Claude Opus | LLM premium | input/output token units and context limits | https://platform.claude.com/docs/en/about-claude/models/overview | Для сложных задач. |
| OpenAI GPT mini / full / premium classes | LLM primary PRD-0 and stronger paths | input/output token units, context and tool terms | https://openai.com/api/pricing/ | Выбирать по quality smoke and policy. |
| OpenAI STT | Transcription fallback | audio minute unit and model availability | https://developers.openai.com/api/docs/pricing | Альтернатива Lemonfox, проверить отдельно. |
| Gemini Flash | LLM, current Gemini candidate | input/output token units, grounding terms and quota | https://ai.google.dev/gemini-api/docs/pricing | Использовать после проверки key/quota. |
| DeepSeek Flash / Pro | LLM low-cost/stronger candidates | cache policy, input/output units, concurrency, alias lifecycle | https://api-docs.deepseek.com/quick_start/pricing | Repo needs operator evidence before use. |
| Lemonfox STT | Speech-to-text | audio-hour unit, upload limits and region terms | https://www.lemonfox.ai/apis/speech-to-text | Provider path already used through server-side boundary. |
| LiteLLM Gateway | Gateway/cost control | gateway deployment/support terms and provider pass-through accounting | https://docs.litellm.ai/docs/proxy/virtual_keys | Не внедрять до решения, что native analytics недостаточно. |
| Infrastructure/storage | Сервер, storage, backups | month/GB/CPU units and retention volumes | local deployment estimates | Уточнить после volume/file-retention decisions. |
