...
- Provider egress: при блокировке прямого egress используется host-local HTTP-to-SOCKS bridge; OpenWebUI получает только HTTP proxy env.
...
- Provider proxy: `OPENWEBUI_OUTBOUND_PROXY` должен быть `http://` или `https://`; direct `socks5h://` в env OpenWebUI не используется.
- SOCKS5 upstream: хранится отдельно в server-local `.env` как input для host-local bridge.
- Bridge port: не public; UFW allow только от Docker subnet к Docker gateway.
...
- `OPENWEBUI_OUTBOUND_PROXY` включает provider egress через HTTP proxy bridge, если direct provider access блокируется.
- `OPENWEBUI_SOCKS5_UPSTREAM` не прокидывается в OpenWebUI; его читает только host-local bridge configuration.
...
- если используется proxy bridge, provider egress из контейнера проверен через HTTP proxy и bridge port не открыт публично;
...
- Direct provider egress может блокироваться provider region policy; PRD-0 решает это штатным HTTP proxy env + host-local SOCKS bridge, без fork/gateway.
...
- Multi-provider env через `OPENAI_API_BASE_URLS`/`OPENAI_API_KEYS` возможен по документации OpenWebUI, но в PRD-0 не выбран как deployment path из-за риска ошибки порядка URL/key и persistent config.
