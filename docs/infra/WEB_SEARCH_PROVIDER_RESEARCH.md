...
| Provider / engine | Commercial/limit signal | Плюсы | Минусы | Вывод |
| --- | --- | --- | --- | --- |
| Brave `brave_llm_context` | Проверять Brave Search plan, request unit, credits/limits вне GitHub | Хороший fit для LLM, меньше отдельного scraping, штатно описан в OpenWebUI | Вероятно нужна учетная запись/API key; для low tier держать concurrency `1` | Первый кандидат для пилота |
...
| Tavily `tavily` | Проверять credits/PAYG model вне GitHub | Самый простой managed старт | Commercial terms зависят от credit accounting; не Google SERP | Хороший fallback без карты |
...
| Serper `serper` | Проверять query credits and package terms вне GitHub | Google SERP на объеме | Это внешний SERP wrapper | Хороший вариант при регулярном использовании |
...
| SearchAPI `searchapi` | Проверять developer/monthly search terms вне GitHub | Managed SERP provider | Нет первого преимущества для простого пилота | Не первый выбор |
| SerpApi `serpapi` | Проверять free tier and starter terms вне GitHub | Зрелый SERP provider, много engines | Требует отдельного procurement review | Только при особой потребности |
| Exa `exa` | Проверять search/request terms вне GitHub | Хорош для AI/coding/search use cases | Не первый базовый вариант | Не first choice |
| Linkup `linkup` | Проверять per-request terms вне GitHub | Агентный web search provider | Не первый базовый вариант | Можно тестировать позже |
