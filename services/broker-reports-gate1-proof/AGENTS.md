...
- Qualified mappings may use only exact title/header/column structure and exact
  ...
  source wording or carry a complete reviewed-schema record. The compiler
  executes this binding and must not infer it from adjacency, uniqueness or row
  values. Broker, year, filename, fuzzy matching, inferred table continuation
  and runtime model calls are forbidden. Unknown/incomplete/unbound rows remain
  `RELEVANT_UNMAPPED` and cannot reach Gate 5.
