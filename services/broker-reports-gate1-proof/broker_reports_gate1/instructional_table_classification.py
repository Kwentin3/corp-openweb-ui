"""Contract for classifying one Canonical table as instructional material.

This module owns only the narrow model wire contract.  The semantic-mapping
owner remains responsible for binding an accepted answer to Canonical and for
building the final table resolution.
"""
...
_STATUSES = frozenset(
    {"INSTRUCTIONAL_REFERENCE", "NOT_INSTRUCTIONAL", "SPECIALIST_REVIEW_REQUIRED"}
)
...
def build_case(*, table: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        set(table) != required
        or not isinstance(table.get("table_ref"), str)
        or not table["table_ref"]
        or not isinstance(context, Mapping)
        or not isinstance(context.get("entries"), list)
    ):
        raise InstructionalClassificationContractError(
            "instructional_classification_table_invalid"
        )
    return {
...
def validate_response(*, response: Any, case: Mapping[str, Any]) -> dict[str, Any]:
    if (
        ...
        raise InstructionalClassificationContractError(
            "instructional_classification_response_invalid"
        )
    ...
    if not isinstance(header_rows, list) or response["header_row"] not in header_rows:
        raise InstructionalClassificationContractError(
            "instructional_classification_header_invalid"
        )
    allowed = {
        (item.get("context_ref"), item.get("relation"))
    ...
    for item in evidence:
        if (
            ...
            or set(item) != {"context_ref", "relation"}
            or (item.get("context_ref"), item.get("relation")) not in allowed
        ):
            raise InstructionalClassificationContractError(
                "instructional_classification_evidence_invalid"
            )
        pairs.append((item["context_ref"], item["relation"]))
    if len(set(pairs)) != len(pairs) or (
        (response["classification"] == "INSTRUCTIONAL_REFERENCE") != bool(pairs)
    ):
        raise InstructionalClassificationContractError(
            "instructional_classification_evidence_coverage_invalid"
        )
