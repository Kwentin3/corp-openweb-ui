...
ORDINARY_TRADE_TAX_MODEL_OPERATION_SET_RESULT_SCHEMA_VERSION = (
...
class OrdinaryTradeTaxModelBridgeRuntime:
    ...
    def run_current_case_operation_set(
        self,
        *,
        ...
        """Aggregate every current disposal in one exact, non-persistent set.

        This is intentionally a bridge-only contract.  The current Fact owner
        chooses the entire disposal set; the existing operation and category
        owners retain all financial calculation and completeness meaning.
        """
        ...
        try:
            ...
            pinned_consumption_sha256 = _sha256_json(pinned_consumption)
            disposal_fact_ids = self._current_disposal_fact_ids(
                consumed=pinned_consumption
            )
            operation_set = _operation_set_identity(
            ...
            if (
                not isinstance(category_scope, dict)
                or category_scope.get("taxpayer_scope_ref")
            ...
            expected_case_binding = {"scope_kind": "case", "scope_id": context.case_id}
            for disposal_fact_id in disposal_fact_ids:
                operation_result = (
                    self._operation_tax_model.run_operation_from_current_source_facts(
                        methodology_ref=operation_methodology_ref,
                        source_fact_methodology_ref=source_fact_methodology_ref,
                        resolved_inputs=copy.deepcopy(resolved_inputs),
                        disposal_fact_id=disposal_fact_id,
                        context=context,
                    )
                )
                operation_consumption = operation_result.get(
                    "source_fact_consumption"
                )
                if not isinstance(operation_consumption, dict):
                    return _operation_set_blocked(
                        reason_code=(
                            "ordinary_trade_tax_model_bridge_operation_set_changed"
                        ),
                        field="source_fact_consumption",
                        owner_classification="INTERNAL_CONTRACT_OR_PIPELINE_DEFECT",
                        owner="OrdinaryTradeTaxModelBridgeRuntime",
                        operation_set=operation_set,
                        taxpayer_binding=validated_taxpayer_binding,
                    )
                case_binding = operation_consumption.get("case_binding")
                if case_binding != expected_case_binding:
                    return _operation_set_blocked(
                        reason_code=(
                ...
                if (
                    _sha256_json(operation_consumption) != pinned_consumption_sha256
                ):
                    return _operation_set_blocked(
                        reason_code=(
                            "ordinary_trade_tax_model_bridge_operation_set_changed"
                ...
                if subject_ref != validated_taxpayer_binding["operation_subject_ref"]:
                    return _operation_set_blocked(
                        reason_code=(
                ...
                operation_ref = "operation-" + disposal_fact_id
                members.append(
                    {
            ...
            final_consumption = self._current_consumption_snapshot(
                source_fact_methodology_ref=source_fact_methodology_ref,
                context=context,
            )
            if _sha256_json(final_consumption) != pinned_consumption_sha256:
                return _operation_set_blocked(
                    reason_code="ordinary_trade_tax_model_bridge_operation_set_changed",
                    field="current_disposal_fact_ids",
                    owner_classification="INTERNAL_CONTRACT_OR_PIPELINE_DEFECT",
                    owner="OrdinaryTradeTaxModelBridgeRuntime",
                    operation_set=operation_set,
                    taxpayer_binding=validated_taxpayer_binding,
                )
            scope_binding = self._category_aggregation.describe_scope(
                scope=category_scope,
                members=members,
            )
            operation_set["scope_binding"] = copy.deepcopy(scope_binding)
        ...
        return _operation_set_result(
            status="proven",
            terminal=ACTIVE_FACT_V2_OPERATION_SET_TO_CATEGORY_TAX_MODEL_PROVEN,
            operation_set=operation_set,
    ...
    def _current_disposal_fact_ids(
        ...
        *,
        ...
        securities = consumed.get("securities")
        if not isinstance(securities, list):
            raise Gate5TaxPeriodCategoryAggregationError(
                "ordinary_trade_tax_model_bridge_operation_set_invalid"
            )
        disposal_fact_ids = sorted(
            str(item.get("disposal_fact_id") or "")
            for item in securities
            if isinstance(item, dict)
            and isinstance(item.get("disposal_fact_id"), str)
        )
        if (
            not disposal_fact_ids
            or len(disposal_fact_ids) != len(set(disposal_fact_ids))
            or any(not _identifier(item) for item in disposal_fact_ids)
        ):
            raise Gate5TaxPeriodCategoryAggregationError(
                "ordinary_trade_tax_model_bridge_operation_set_invalid"
            )
        return tuple(disposal_fact_ids)

    def _current_consumption_snapshot(
        self,
        *,
        source_fact_methodology_ref: dict[str, Any],
        context: ArtifactAccessContext,
    ) -> dict[str, Any]:
        consumed = self._current_facts.run(
...
def _operation_set_identity(
    *,
    disposal_fact_ids: tuple[str, ...],
    source_fact_consumption_sha256: str,
    context: ArtifactAccessContext,
) -> dict[str, Any]:
    values = list(disposal_fact_ids)
    identity = {
        "schema_version": "broker_reports_ordinary_trade_operation_set_v1",
        "case_binding": {"scope_kind": "case", "scope_id": context.case_id},
        "disposal_fact_ids": values,
        "operation_refs": ["operation-" + item for item in values],
        "source_fact_consumption_sha256": source_fact_consumption_sha256,
    }
    return {
        **identity,
        "disposal_fact_ids_sha256": _sha256_json(values),
        "scope_binding": None,
    }


def _operation_set_result(
    *,
    status: str,
    terminal: str,
    operation_set: dict[str, Any] | None,
    operation_results: list[dict[str, Any]],
    category_result: dict[str, Any] | None,
    taxpayer_binding: dict[str, Any] | None,
    blockers: list[dict[str, Any]],
    ...
    return {
        "schema_version": ORDINARY_TRADE_TAX_MODEL_OPERATION_SET_RESULT_SCHEMA_VERSION,
        "status": status,
        "terminal": terminal,
        "route": [
            "OrdinaryTradeCandidateRuntimeFactory.create",
        ...
        "operation_set": copy.deepcopy(operation_set),
        "operation_results": copy.deepcopy(operation_results),
        "category_result": copy.deepcopy(category_result),
        "taxpayer_binding": copy.deepcopy(taxpayer_binding),
        "blockers": copy.deepcopy(blockers),
        "demands": copy.deepcopy(demands),
