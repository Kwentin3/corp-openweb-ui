...
def is_fns_2ndfl_neutral_event_source_unit(value: Any) -> bool:
    """Recognize the FNS family without treating every neutral XML as FNS."""

    if not isinstance(value, dict):
        return False
    if value.get("parser") != "python_expat_neutral_events":
        return False
    if (value.get("source_location") or {}).get("kind") != "xml_neutral_event_rows":
        return False
