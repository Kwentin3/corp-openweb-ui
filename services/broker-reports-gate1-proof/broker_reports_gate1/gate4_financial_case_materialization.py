...
GATE4_FINANCIAL_CASE_FACT_SCHEMA_VERSION = (
    "broker_reports_gate4_financial_case_fact_v1"
)
FACTORY_REQUIRED = (
    "Gate4FinancialCaseMaterializerFactory.create is the only deterministic "
    "FinancialAnnotationsV2 to Gate4FinancialCaseFactV1 entrypoint"
)
FORBIDDEN = (
    ...
    def _materialize_annotation(
        *,
        ...
        if (
            ...
            or tuple(item.get("role") for item in role_bindings)
            != expected_roles
        ):
            raise Gate4FinancialCaseMaterializationError(
                "gate4_role_profile_mismatch"
            )
...
def _case_binding(context: ArtifactAccessContext) -> dict[str, str]:
    if not isinstance(context, ArtifactAccessContext):
        raise Gate4FinancialCaseMaterializationError(
            "gate4_trusted_context_required"
        )
    if not context.user_id or not context.allow_private:
        raise Gate4FinancialCaseMaterializationError(
            "gate4_private_context_required"
        )
    if context.case_id:
    ...
    raise Gate4FinancialCaseMaterializationError(
        "gate4_case_or_chat_scope_required"
    )
...
def _normalize_role_value(role: str, source_literal: str) -> str:
    ...
    if role in {"asset", "currency"} and value:
        return value
    raise Gate4FinancialCaseMaterializationError(
        "gate4_role_value_invalid"
    )
