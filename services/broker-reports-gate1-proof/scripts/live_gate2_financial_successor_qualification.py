#!/usr/bin/env python3
"""Qualify GPT-5.4 Nano on the frozen Financial Evidence successor workload."""
...
FINANCIAL_SUCCESSOR_QUALIFICATION_SCHEMA_VERSION = (
    "broker_reports_gate2_financial_successor_qualification_v1"
)
FINANCIAL_SUCCESSOR_QUALIFICATION_POLICY_VERSION = (
    "gate2_financial_successor_exact_model_qualification_v1"
)
...
FACTORY_REQUIRED = (
    ...
    "Gate2SuccessorArtifactFamilyFactory are the only authorization, live "
    "execution, product validation and artifact construction entrypoints"
)
FORBIDDEN = (
    "This qualification harness must not use customer data, production "
...
class SuccessorQualificationCase:
    case_id: str
    features: tuple[str, ...]
...
@dataclass(frozen=True)
class SuccessorQualificationFixture:
    manifest_file_sha256: str
    manifest_canonical_hash: str
    local_proof_receipt: dict[str, Any]
    registry: Gate2FinancialEvidenceRegistrySnapshot
    cases: tuple[SuccessorQualificationCase, ...]


@dataclass(frozen=True)
class _SuccessfulCase:
    case: SuccessorQualificationCase
...
class _NoCallClient:
    async def extract(self, **_kwargs):
        raise AssertionError("qualification_preflight_must_not_call_provider")
...
def main() -> int:
    ...
    if (
        ...
        and not receipt_path.name.endswith(".safe.json")
    ):
    ...
    preflight_cases = successor_preflight_cases(
        fixture=fixture,
    ...
    estimated_cost = sum(
        (
            Decimal(
                str(item["schema_dry_build"]["estimated_cost_usd"])
            )
            for item in preflight_cases
        ),
        Decimal("0"),
...
def build_successor_qualification_fixture(
    manifest_path: Path = DEFAULT_MANIFEST_PATH,
) -> SuccessorQualificationFixture:
    manifest_bytes = manifest_path.read_bytes()
    manifest_file_sha256 = hashlib.sha256(manifest_bytes).hexdigest()
    manifest = json.loads(manifest_bytes.decode("utf-8"))
    registry = Gate2FinancialEvidenceRegistryFactory().create()
    local_proof = Gate2SuccessorLocalProofFactory(
        registry=registry
    ).create(manifest=manifest)
    if local_proof["status"] != "passed":
        raise ValueError("successor_qualification_local_proof_failed")
    ...
    for case in manifest["cases"]:
        ...
        if len(batch.scopes) != 1:
            raise ValueError(
                "successor_qualification_scope_count_invalid"
            )
    ...
    return SuccessorQualificationFixture(
...
def successor_preflight_cases(
    *,
    fixture: SuccessorQualificationFixture,
    model_id: str,
) -> list[dict[str, Any]]:
...
async def qualify_successor_model(
    *,
    model_client,
    model_id: str,
    fixture: SuccessorQualificationFixture,
    ...
    successful: list[_SuccessfulCase] = []
    provider_calls = 0
    input_tokens = 0
    output_tokens = 0
    ...
    def current(*, terminal: bool) -> dict[str, Any]:
        return _successor_execution_result(
            fixture=fixture,
    ...
    for case in fixture.cases:
        provider_calls += 1
        execution_ref = (
            f"execution:successor-qualification:{case.case_id}"
        )
        decision_validation_ref = (
            f"validation:successor-qualification:{case.case_id}"
        )
        try:
            ...
            if (
                ...
                raise ValueError(
                    "successor_qualification_budget_receipt_missing"
                )
...
def _successor_execution_result(
    *,
    fixture: SuccessorQualificationFixture,
    successful: list[_SuccessfulCase],
    case_receipts: list[dict[str, Any]],
    ...
    if terminal and len(successful) == len(fixture.cases):
        product_proof = _terminal_product_proof(
            fixture=fixture,
            successful=successful,
        )
    four_dispositions = (
...
def _terminal_product_proof(
    *,
        ...
        observations=(
            Gate2SuccessorScopeObservation(
                ...
                materialized_artifact=(
                    item.result.materialized_artifact
                ),
    ...
    artifact_family = Gate2SuccessorArtifactFamilyFactory(
        registry=fixture.registry
    ).create(
        run_ref="run:successor-qualification:gpt54-nano",
        source_extraction_run_ref=(
            "run:synthetic-source:successor-qualification"
        ),
        inputs=(
            Gate2SuccessorArtifactInput(
    ...
    compatibility_reads = [
        reader.read(
            artifact_ref=(
                f"artifact:successor-qualification:{item.case.case_id}"
            ),
            payload=item.result.materialized_artifact,
        )
        for item in successful
    ]
    ...
    return {
        ...
        "financial_context_integrity_hash": context[
            "integrity_hash"
        ],
        "raw_provider_output_included": False,
    }
...
def _actual_model_output(
    *,
    result: Gate2FinancialEvidenceSuccessorResult,
    registry: Gate2FinancialEvidenceRegistrySnapshot,
) -> dict[str, Any]:
    decision = result.validated_decision.decision
    if isinstance(decision, TypedFinancialInputDecision):
        declaration = registry.get(decision.input_type_id)
        values = {
            role: None
            for role in (
        ...
        for binding in decision.value_bindings:
            values[binding.role_id] = binding.source_value_ref
        return {
            "decision": {
    ...
    if isinstance(decision, UnclassifiedFinancialInputDecision):
        return {
            "decision": {
                "disposition": decision.disposition,
                "value_bindings": [
                    ...
                    for item in decision.value_bindings
                ],
                "reason_code": decision.reason_code,
            }
        }
    ...
    raise ValueError("successor_qualification_decision_type_invalid")


def successor_qualification_contract_identity(
    *,
    fixture: SuccessorQualificationFixture,
) -> Gate2EconomyQualificationContractIdentity:
    ...
    provider_schema_hash = _sha256_json(
        [
            case.scope.decision_contract.provider_schema_hash("openai")
            for case in fixture.cases
        ]
    )
    ...
    return Gate2EconomyQualificationContractIdentity(
        ...
        input_contract_version=(
            f"{DETERMINISTIC_FINANCIAL_SCOPE_SCHEMA_VERSION}:"
            f"{fixture.registry.registry_hash}:"
            f"{fixture.manifest_canonical_hash}"
        ),
        output_contract_version=(
            f"{DECISION_SCHEMA_VERSION}:{provider_schema_hash}"
        ),
        prompt_version=(
            f"{SUCCESSOR_PROMPT_CONTRACT_ID}:{prompt.hash}"
        ),
    ...
    *,
    ...
    return {
        **identity.to_dict(),
        ...
        "successor_prompt_contract": SUCCESSOR_PROMPT_CONTRACT_ID,
        ...
        "artifact_policy": SUCCESSOR_ARTIFACT_POLICY_VERSION,
...
def _subject(model_id: str) -> dict[str, str]:
    return {
        ...
        "provider_profile_id": PROVIDER_PROFILE_ID,
        "workload_class": "gate2_financial_evidence_successor",
    }
