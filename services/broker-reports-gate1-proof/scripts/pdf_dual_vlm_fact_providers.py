...
GEMINI_RESEARCH_MODEL_ALLOWLIST = frozenset(
    {
        "models/gemini-3.1-flash-lite",
        DEFAULT_GEMINI_MODEL_ID,
    }
)
OPENAI_RESEARCH_MODEL_ALLOWLIST = frozenset(
    {
...
class PdfDualVlmFactProviderFactory:
    ...
    def _validate_config(self) -> None:
        if self.config.gemini_model_id not in GEMINI_RESEARCH_MODEL_ALLOWLIST:
            raise PdfDualVlmFactProviderError(
                "pdf_dual_vlm_gemini_model_not_allowlisted",
                "provider_configuration",
            )
        if self.config.openai_model_id not in OPENAI_RESEARCH_MODEL_ALLOWLIST:
            raise PdfDualVlmFactProviderError(
    ...
    def qualify(self) -> dict[str, Any]:
        ...
        return {
            ...
            "image_input_supported": (
                self.config.openai_model_id in OPENAI_RESEARCH_MODEL_ALLOWLIST
            ),
