...
async function waitForTurn(page) {
  const terminalMessage = page.locator('[role="listitem"]').last().filter({
    hasText: /Расчёт остановлен|Расчётный черновик готов|3-НДФЛ XML подготовлен|Подготовка остановлена|Подготовка приостановлена|Анализ завершён|Продажа найдена|В отчёте есть позиция|Не удалось достоверно|Готов только анализ|Неподаваемый черновик/,
  });
