...
def test_snapshot_validator_accepts_only_the_closed_v19_same_cell_evidence_identity():
    snapshot = OrdinaryTradeMappingManagedPrompt(
        prompt_ref="test-v19-prompt",
        command=ORDINARY_TRADE_MAPPING_V19_PROMPT_COMMAND,
        version="test-v19-version",
        content="Map {{ordinary_trade_mapping_case_json}}.",
        hash="e" * 64,
        source="test",
        ...
        tags=(ORDINARY_TRADE_MAPPING_V19_PROMPT_REQUIRED_TAG,),
        safe_metadata={},
    ).snapshot()

    assert validate_ordinary_trade_mapping_prompt_snapshot(snapshot) == snapshot
    mixed = dict(snapshot)
    mixed["prompt_command"] = ORDINARY_TRADE_MAPPING_V18_PROMPT_COMMAND
    with pytest.raises(OrdinaryTradeMappingPromptError) as invalid:
        validate_ordinary_trade_mapping_prompt_snapshot(mixed)
    assert invalid.value.code == "ordinary_trade_mapping_prompt_snapshot_invalid"


def test_snapshot_validator_accepts_only_the_closed_v22_currency_binding_identity():
    snapshot = OrdinaryTradeMappingManagedPrompt(
        prompt_ref="test-v22-prompt",
        command=ORDINARY_TRADE_MAPPING_V22_PROMPT_COMMAND,
        version="test-v22-version",
        content="Map {{ordinary_trade_mapping_case_json}}.",
        hash="f" * 64,
        source="test",
        ...
        tags=(ORDINARY_TRADE_MAPPING_V22_PROMPT_REQUIRED_TAG,),
        safe_metadata={},
    ).snapshot()

    assert validate_ordinary_trade_mapping_prompt_snapshot(snapshot) == snapshot
    mixed = dict(snapshot)
    mixed["prompt_command"] = ORDINARY_TRADE_MAPPING_V19_PROMPT_COMMAND
    with pytest.raises(OrdinaryTradeMappingPromptError) as invalid:
        validate_ordinary_trade_mapping_prompt_snapshot(mixed)
    assert invalid.value.code == "ordinary_trade_mapping_prompt_snapshot_invalid"


def test_snapshot_validator_accepts_only_the_closed_v23_continuation_claim_identity():
    snapshot = OrdinaryTradeMappingManagedPrompt(
        prompt_ref="test-v23-prompt",
        command=ORDINARY_TRADE_MAPPING_V23_PROMPT_COMMAND,
        version="test-v23-version",
        content="Map {{ordinary_trade_mapping_case_json}}.",
        hash="0" * 64,
        source="test",
        ...
        tags=(ORDINARY_TRADE_MAPPING_V23_PROMPT_REQUIRED_TAG,),
        safe_metadata={},
    ).snapshot()

    assert validate_ordinary_trade_mapping_prompt_snapshot(snapshot) == snapshot
    mixed = dict(snapshot)
    mixed["prompt_command"] = ORDINARY_TRADE_MAPPING_V22_PROMPT_COMMAND
    with pytest.raises(OrdinaryTradeMappingPromptError) as invalid:
        validate_ordinary_trade_mapping_prompt_snapshot(mixed)
    assert invalid.value.code == "ordinary_trade_mapping_prompt_snapshot_invalid"
