...
class BrokerReportsGate2Fns2NdflAdapterTest(unittest.TestCase):
    ...
    def test_unknown_xml_never_falls_back_to_model(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(readiness.validation["validator_status"], "failed")
            self.assertIn(
                "fns_2ndfl_schema_family_unknown",
                readiness.validation["error_code_counts"],
            )
            terminal = readiness.packages[0]["typed_adapter_terminal"]
            self.assertEqual(terminal["provider_calls"], 0)
            self.assertEqual(terminal["provider_tokens"], 0)
            self.assertEqual(terminal["provider_cost"], 0)
