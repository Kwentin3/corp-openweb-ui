...
def test_doc27_current_contracts_bind_the_frozen_inventory_and_read_boundary() -> None:
...
def test_doc27_compatibility_contracts_are_explicit_and_consumer_scoped() -> None:
    assert len(WAVE0_MAPPINGS) == 4
    assert len({mapping.feature_flag for mapping in WAVE0_MAPPINGS}) == 4
    assert all(
...
def test_doc27_does_not_wire_compatibility_adapters_into_product_consumers() -> None:
    product_paths = (
