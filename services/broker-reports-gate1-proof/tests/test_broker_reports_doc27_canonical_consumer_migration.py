...
def test_01_frozen_inventory_accounts_all_17_surfaces_once() -> None:
    assert len(FROZEN_CONSUMER_SURFACES) == 17
    assert len({item.consumer_id for item in FROZEN_CONSUMER_SURFACES}) == 17
    assert len({item.source_file for item in FROZEN_CONSUMER_SURFACES}) == 17
    assert all(item.consumer_class != "UNRESOLVED" for item in FROZEN_CONSUMER_SURFACES)
    assert sum(item.consumer_class == "WAVE_0_TEST" for item in FROZEN_CONSUMER_SURFACES) == 3
...
def test_02_wave0_mappings_are_explicit_versioned_and_consumer_specific() -> None:
    assert len(WAVE0_MAPPINGS) == 4
    assert len({item.feature_flag for item in WAVE0_MAPPINGS}) == 4
...
def test_04_flag_off_and_missing_active_are_explicit_without_fallback() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        ledger = CanonicalReadLedger()
        disabled = Doc22SafeEvidenceCanonicalAdapterFactory(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        missing = Doc22SafeEvidenceCanonicalAdapterFactory(
