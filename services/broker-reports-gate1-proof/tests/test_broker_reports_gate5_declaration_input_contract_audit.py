...
def test_current_foreign_tax_consumer_accepts_broker_evidence_without_netting() -> None:
    ...
    rule = next(
        ...
        if row["rule_id"] == "foreign-tax-credit-articles-214-232-v2"
    )
    ...
    assert "broker statement is sufficient source evidence" in rule[
        "deterministic_rule"
    ]
