...
def test_prepare_video_audio_extracts_only_first_audio_track(monkeypatch):
    calls = []
    ...
    result = prepare_video_audio(source_bytes=b"video", source_filename="call.mp4", output_profile=OutputProfile.MP3_HIGH_COMPAT)

    assert result.filename == "call.mp3"
    assert result.mime_type == "audio/mpeg"
    assert result.audio_bytes == b"prepared-mp3"
    assert result.duration_seconds == 12.5
    assert ["-map", "0:a:0"] == calls[1][calls[1].index("-map"):calls[1].index("-map") + 2]
    assert "-vn" in calls[1]


def test_prepare_video_audio_fails_closed_when_video_has_no_audio(monkeypatch):
    monkeypatch.setattr(
    ...
    with pytest.raises(MediaPreparationError, match="Video has no audio stream") as raised:
        prepare_video_audio(source_bytes=b"video", source_filename="silent.mp4", output_profile=OutputProfile.MP3_HIGH_COMPAT)
